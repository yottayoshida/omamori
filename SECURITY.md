...
**Two carriers, one hashed and one not (#177 B2)**: the audit log now records the wrapper basename twice — as the `:{wrapper}` suffix of `detection_layer` (hashed, part of `HashableEvent`) and as the standalone `AuditEvent.wrapper_kind` field (**not** hashed; `CHAIN_VERSION` stays at `1` and `HashableEvent` does not include it — same Design A cost as [Process Provenance](#process-provenance-v0131-420)'s fields). A same-user attacker can alter `wrapper_kind` on any entry without breaking the hash chain; `detection_layer`'s copy of the same fact remains chain-protected. Chain-integrity coverage for `wrapper_kind` is tracked for a future `CHAIN_VERSION` bump (issue #177 B3) — do not drop `detection_layer`'s `:{wrapper}` suffix (tracked separately for sunset in #459) until that bump lands, or wrapper attribution loses tamper-evidence entirely.
