...
| `curl URL \| env bash` / `curl URL \| sudo bash` | **Scheduled to close in v0.9.5 (#146 P1-1).** Pipe-to-shell detection will run before transparent-wrapper unwrapping, covering 7 wrappers (`sudo`, `env`, `nice`, `timeout`, `nohup`, `exec`, `command`) including chained wrappers, absolute-path variants (`/usr/bin/env`, `/bin/sudo`), and flags (`-E`, `-u`, `-i`, `-u USER`). False-positive guard via `has_positional_script` heuristic preserves `bash script.sh`, `env VAR=1 bash script.sh` etc. as safe. **Until v0.9.5 ships**, the gap remains as documented in `src/unwrap.rs::tests::{curl_pipe_env_bash_not_yet_blocked,echo_pipe_sudo_bash_not_yet_blocked}`. |
