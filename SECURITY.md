...
| `curl URL \| bash -c 'source /dev/stdin'` (shell launcher reading piped payload) | v0.9.6 | Pipe-RHS shell launchers with `-c 'source /dev/stdin'` (and the `source` / `eval` / `exec` family inside shell launchers reading `/dev/stdin`) are blocked at the launcher boundary. The non-pipe common case `bash -c 'source /dev/stdin' < file` (explicit stdin redirect) remains Allow. See `tests/hook_integration.rs` corpus entry `pipe-launcher-source-stdin-block`. PR2 ([#184](https://github.com/yottayoshida/omamori/pull/184)) scope 6. |
...
|---------------|-----------------|
| `sudo rm -rf` (direct path, not piped) | sudo changes PATH before shim runs; shim is never invoked. (Note: `curl URL \| sudo bash` pipe-RHS is closed — see A above.) |
