...
- ~~**Four more printers on the same path are still unconditional**~~
  ([#518](https://github.com/yottayoshida/omamori/issues/518)) — **closed.** All four now go
  through the throttle, each with its own sentinel kind. The **AI gate was deliberately not
...
| **T4: Git status spoofing** | 7.2 | `GIT_DIR`, `GIT_WORK_TREE`, `GIT_INDEX_FILE`, `GIT_COMMON_DIR` removed from git subprocess. The command itself still follows them and its own global options, so when any of these variables or any global option is present the no-changes downgrade is not applied at all |
...
| `parent_process` | Resolved exec path of the parent process (`proc_pidpath`), plaintext, characters that reorder or conceal text substituted (#515) |
...
The rule is now stated rather than enumerated: a character is replaced when it **draws nothing itself and can move or conceal the text around it**. That covers `Cc`, the bidirectional controls and isolates (U+061C, U+200E–U+200F, U+202A–U+202E, U+2066–U+2069), the zero-width and invisible separators (U+00AD, U+180E, U+200B–U+200D, U+2060, U+FEFF), and the tag characters (U+E0000–U+E007F). `provenance::sanitize` applies the same rule to `parent_process` from one shared definition; what stays local to it is its length cap.

**Characters with a composing role are outside the rule**, because reordering or concealment is not their only effect — they are how legitimate text is built. Variation selectors (U+FE00–U+FE0F, U+E0100–U+E01EF) change how the preceding single character is drawn. U+200C and U+200D join their neighbours into one glyph: `👩‍💻` is a joiner sequence, as is every family emoji, and both are orthographic in Persian, Arabic and several Indic scripts. Substituting either turns a legitimate filename into mojibake, and a sanitizer widened until an operator cannot read the path they must act on has traded one failure for another. U+200B stays in scope — it composes nothing, and invisible padding is how two different strings are made to look like one.

**Residual — this covers two strings, not every string omamori prints.** The rule is applied where `AuditSummary::path_error` is built and where `parent_process` is collected. It is **not** applied to every rendered surface: `audit show` prints parsed event fields as they are stored, and the key-store warnings interpolate an `io::Error` whose text can carry a path. A same-user attacker with direct write access to `audit.jsonl` can hand-craft a line whose fields hold these characters raw, and reading that file with any tool — omamori's own `show` included — will render them. Widening the coverage is a separate question from the one #515 asked; what closed here is the two paths that reach a consent prompt and `status`.
...
If a previous write was interrupted (partial JSON line), `append()` detects the unterminated line and inserts a newline before writing the new entry. `read_chain_state()` walks backward from the end of the file, one line at a time and up to 64 MiB, and stops at the first line carrying a `chain_version` — the last chain entry. When that span holds none, it refuses rather than starting a new chain. Lines that are not chain entries (a torn fragment, foreign JSON, a legacy-shaped line, a chain-shaped line missing the `seq` or `entry_hash` omamori always writes) are read past; see [Forward-Unknown Chain Versions](#forward-unknown-chain-versions-177) for what that closes and what it leaves open ([#465](https://github.com/yottayoshida/omamori/issues/465)).
...
| `read_hwm()` | audit.jsonl.hwm | `ELOOP` at the open; a FIFO or directory there is refused the same way. All three are `HwmUnusable::NotAMark` — tamper evidence, exit 3, no truncation comparison |
| `read_hwm()` classification (#491) | audit.jsonl.hwm | Refusal already done; a `stat` **after** the failed open names which shape was found, so the message stops naming causes it did not observe. **The classification decides wording only** — no production branch reads the variant, so a value an attacker can choose never reaches a verdict |
...
- *`read_hwm()` gained a `stat` in #491, and it runs **after** the open has already failed — the refusal is complete before it is reached, and it exists only to say which shape was found ("it is a symlink" / "it is a directory") instead of naming two causes for every failure. It is deliberately not an errno test: `O_NOFOLLOW`'s value is not guaranteed to match across the two platforms omamori ships on, and `InvalidInput` would only be `reject_non_regular` recognising a string it built itself. An attacker who swaps the path between the failed open and this `stat` changes the wording of one sentence; the verdict is exit 3 either way.*
...
Entries written before v0.7.0 lack chain fields. `append()` reads past a legacy last entry — as it reads past anything that is not a chain entry — and continues from the log's last chain entry; it starts a new chain from genesis (`seq=0`) only when the log holds no chain entry at all, and refuses when its 64 MiB tail scan ends without finding one ([#465](https://github.com/yottayoshida/omamori/issues/465), described under [Forward-Unknown Chain Versions](#forward-unknown-chain-versions-177)). Through `1.1.0` a legacy last entry started a new chain from genesis on its own. `omamori audit verify` skips legacy entries with a warning. A log containing only legacy entries returns exit code 2 (no chain entries to verify). A legacy entry appearing *after* real chain entries have started (not at the head) fails closed as `broken_at` (#177): legacy entries never participate in `prev_hash`/`seq` continuity tracking, so an attacker could otherwise splice unaudited content into the middle of an otherwise-verified chain by making it look legacy. Note this fail-close is deliberately blind to *why* the entry looks legacy — a genuine downgrade (running an older omamori binary against a chain a newer one had already extended) produces the identical shape and is not distinguished from an attack; see Forward-Unknown Chain Versions below for the analogous, and equally undistinguishable, upgrade-direction case.

### Forward-Unknown Chain Versions (#177+)

An entry declaring a `chain_version` this binary doesn't recognize (a future format, or a downgrade after a newer entry was written) is *not* treated as tampering. `omamori audit verify` reports it distinctly and exits with code **4**, stopping verification at that entry — everything from it onward is counted but not trusted (the `prev_hash` chain running through an unauthenticated entry can't be relied on for anything after it either). `omamori report`/`doctor` surface this the same way, with wording that does not claim tampering. `append()` similarly refuses to write when the log's *last chain entry* — the last line carrying a `chain_version`, however much non-chain content follows it ([#465](https://github.com/yottayoshida/omamori/issues/465); the paragraph after the residual risk below) — has an unrecognized `chain_version`: it returns an error, so `[audit] strict = true` blocks the triggering command the same way it blocks on any other unrecordable event; in the default (non-strict) mode, every existing call site's own error handling turns this into a stderr warning only, never affecting the command's own block/allow decision (G-2, best-effort). **This warning is throttled on the Layer 1 (PATH shim) path** (`try_audit_append`, at most once per 5 minutes — pre-existing behavior shared by every append failure reason, not specific to this case), so a persistent unrecognized-`chain_version` tail (e.g. after a downgrade following a chain a newer omamori extended, or in an environment mixing binaries across the version boundary) surfaces one warning and then goes silent for up to 5 minutes at a time even though every intervening command's audit event is still silently dropped; the Layer 2 (hook) path (`warn_audit_append_error`, the break-glass site) is not throttled and warns on every failure. QA Phase 8 (#177 B3) confirmed this experimentally: a rapid sequence of Layer 1 commands against an unrecognized-version tail produced only the first warning, not one per command.

**What counts as such an entry ([#556](https://github.com/yottayoshida/omamori/issues/556)).** It is decided from `chain_version` alone, through the same peek in `verify`, `append()` and the prune scan ([What a Prune Records](#what-a-prune-records-about-what-it-removed-461)): a JSON object whose `chain_version` is a `u32` this binary does not support — whatever else the line holds, including a `seq` of any type or none. A line whose `chain_version` is not a `u32` or is given twice is not such an entry. A line that is not a JSON object is not an event to any reader of the log: `verify` counts it as torn, and `append()`, the prune scan, `audit show` and `report` read it as nothing, so a line `verify` counts as torn is never one `audit show` displays or `report` counts. Through `1.2.0` the three read the line differently. `verify` read `chain_version` and `seq` together, so an entry whose `seq` did not read as an unsigned 64-bit integer counted as a torn line: `verify` reported the log intact, `report` said `intact` and `doctor` said nothing, while `append()` refused to write behind the same line and every later event went unrecorded. Every reader also took a JSON array positionally, as a derived `Deserialize` does: `append()` refused behind `[999]`, `verify` reported `[999,7]` as exit 4, and a whole entry written as an array was verified, shown and counted like the object it spelled. The prune scan read `serde_json::Value`, which counted a line naming `chain_version` twice.

**Residual risk — exit 4 can be attacker-induced to soften real tampering.** Because a genuine future `chain_version` and a forged one are indistinguishable without a signature this binary can't verify, an attacker with `audit.jsonl` write access can plant one line ahead of an actual deletion/reorder — or, as of `chain_version` 2 (#177 B3), ahead of a `pid`/`ppid`/`parent_process`/`cwd_hash`/`wrapper_kind` value tampering on an otherwise-hashed v2 entry — to downgrade the report from exit 1 ("chain broken … may have been tampered with") to exit 4 ("unrecognized chain_version … not necessarily tampering"). This does **not** destroy evidence: the high-water-mark file is left untouched (not bootstrapped, not lowered) rather than silently advanced past the gap, `report`/`doctor` are non-quiet either way, and removing the planted line restores full detection of the underlying tamper. Tail truncation is deliberately left off that list: since #470 the mark is still **compared** in this state, so a removed tail behind the planted line is reported as truncation (exit 3) rather than downgraded — **provided a line at or after the planted one states a `seq` that reads as an unsigned 64-bit integer**. When none does, there is no end to compare and the verdict is exit 4 with the removal unreported, which one line ending the file is enough to arrange — see [Truncation Detection Across a Halt](#truncation-detection-across-a-halt-470). The attacker also cannot mask anything that happened *before* the planted line — `broken_at` still wins if the real tamper is earlier in the file. This is the accepted cost of forward compatibility, not a defect to be patched away: refusing to verify past a version boundary the binary can't authenticate is the same design principle that makes the "unrecognized version" case safe in the first place. Operators: if `omamori audit verify` reports exit 4 and you are already running the latest omamori, treat it as possible tampering, not merely a version mismatch — the CLI reflects this same caution (`omamori audit verify --help` / the exit-4 message itself).

**Where `append()` picks up the chain (#465).** Content after the log's last chain entry that is not itself shaped like a chain entry — a torn line, foreign JSON, a JSON value that is not an object (through `1.2.0` an array such as `[999]` was read positionally as declaring that version, #556), a legacy-shaped line, a line whose `chain_version` is not a `u32` (a string, `null`, a number beyond the type), a chain-shaped line whose `seq` or `entry_hash` is missing or of the wrong type, or padding — never makes `append()` start a second chain or lift a refusal. `append()` walks backwards from the end of the file one line at a time and stops at the first line carrying a `chain_version`; what lies in between is read past. The walk covers the last 64 MiB: when no chain entry *starts* within them, `append()` refuses — an error, so `[audit] strict = true` blocks the command, and in the default non-strict mode the event goes unrecorded with a stderr warning, throttled on the Layer 1 path to one per five minutes like every other append failure — rather than reading further or restarting. Only a log with no chain entry anywhere, and shorter than that, starts a chain from genesis. Through `1.1.0` the search stopped at a 64 KB window and treated anything it could not read as a chain entry as "no chain here": non-JSON padding pushed a planted entry out of the window and let appends resume, one foreign line such as `{"pad":1}` did the same without any padding, and — with no attacker involved — a genuine entry longer than the window (the hook path records the whole command text) made the next append restart at `seq 0` and `omamori audit verify` report a chain nobody had touched as broken.

**Why the walk is limited.** Every byte read past is time `hook-check` spends before it can print its deny, and a hook stopped by the host's timeout loses the deny — the failure the audit lock's bounded acquisition already exists to prevent. Measured on a release build against a copy of a four-month log: 63 MiB of two-byte lines, the costliest shape per byte, is read past in 1.3 s; 1 GiB of the same is refused in 1.3 s, and a single unbroken 1 GiB line in 0.07 s; peak memory stayed near 5 MB throughout, since the scan holds one chunk and keeps no line — the exception is the `entry_hash` of the entry it stops at, which it has to hold to chain onto, and which is 64 hex characters on anything omamori wrote. The longest line omamori wrote to that log was 46 KB, and the shim path records only a program name, so the limit is far from anything written in normal use — but it is a limit on bytes, not a guarantee about writers: an entry longer than 64 MiB (a local process feeding `hook-check` that much input, which is recorded in full) makes appends refuse until it is moved out of the log, and so does a log from before chain tracking (v0.7.0) holding more than 64 MiB of legacy entries and nothing else.

**What this does not change.** (1) `append()` does not authenticate the line it stops at. A line shaped like a supported chain entry (`chain_version`, `seq`, a non-empty `entry_hash`) is taken at its word, so a same-user attacker who can write one well-formed line can still lift a refusal or redirect the chain; `verify` reports that line as a break, or past a halt only counts it. Closing this would need the append path to authenticate the tail against the keyring. (2) `verify`'s verdicts do not move. It counts non-JSON and non-entry JSON as torn lines and keeps going, and it still fails closed on a legacy-shaped line after the chain has started and on a chain-shaped line whose fields do not check out; `append()` reading past such a line changes what the next entry links to, not what `verify` says about the line. A store carrying a planted unrecognized-`chain_version` entry reports exit 4 whatever follows it, unless the last `seq` stated at or after it — read as an unsigned 64-bit integer — is below the high-water mark, which is reported as truncation (exit 3, [#470](#truncation-detection-across-a-halt-470)); one carrying only non-chain content after its last entry reports intact (exit 0) with the torn lines counted. (3) The cost persists. The audit prune rewrites the head of the log and keeps its tail, so non-chain content stays until an operator removes it: within the limit every append pays up to the time above, and beyond it every append is refused.
...
Whether a successor exists is decided from `seq` alone, and **before** the check on `entry_hash`'s shape. That order is load-bearing. Through `1.1.0` a malformed entry was answered by restarting the chain from genesis, which numbers the next entry `0` — so a line combining `seq: u64::MAX` with an unusable `entry_hash` would otherwise have taken the restart path and reached the same `seq: 0` outcome the refusal exists to prevent, without ever passing through the increment. Since #465 such a line is read past rather than restarted from, and the order still decides the verdict: the refusal fires on the number whatever the `entry_hash` looks like, including shapes that are not strings at all.
...
**What the refusal prevents.** Without it, the next entry is numbered `0` — that was the shipped behaviour before `[profile.release]` set `overflow-checks = true`, and the flag alone would only have converted it into a panic, which is a different wrong answer for a store that is intact. A `seq` of `0` against a populated high-water-mark takes the truncation-detection path: `append()` warns that the tail *may have been truncated* when nothing was removed, and `omamori audit verify` compares the same way. In the narrower case where the `.hwm` sidecar cannot be read as a mark (`HwmState::Unusable`), the wrapped value is written back as the new mark, lowering it to `0` and leaving later real truncation undetectable against it. **Which sidecar shapes actually reach that outcome is decided on the write side, not by how the read classified them** (measured on `8d51fe6`): a sidecar this process cannot read, a FIFO, a socket, and content that is not a plain integer are all replaced, because `write_hwm` publishes by `rename` and `rename` lands on all of them. Two do not: a **symlink**, refused by `write_hwm`'s explicit `reject_symlink` check before the publish — `rename` itself would succeed and replace the link, so the guard is what prevents it, not the syscall — and a **directory**, where `rename` fails with `EISDIR`. Since #490 the sentences `audit verify` prints say which of the two happened rather than asserting the reset unconditionally. Neither outcome requires the attacker to hold the HMAC key, which is what made this worth closing before 1.0 rather than documenting as residual.
...
- The **comparison** runs whatever halted, against the last `seq` *stated* by a line at or after the halting line — that line included, and unverified by definition. Only a stated value counts: a line that omits `seq`, or states one that does not read as an unsigned 64-bit integer, is reported at the position it should have occupied, and that number is one past the last verified entry, so using it as an end would compare the mark against the halt point instead of the file. When nothing at or after the halt states a `seq` there is no end and the comparison is skipped, which is what a future format that renames the field would produce. When the comparison does fire, `tail_truncated` outranks the halt on all three surfaces that render a verdict: `audit verify` exits 3 rather than 2 or 4, `report --json` reports `truncated`, and `doctor` prints the truncated line. Substituting one verdict for another was the point of provoking the halt, so leaving any surface behind would have left the substitution working there.
...
**Residual risk — the compared end is not authenticated.** An attacker who deletes the tail *and* renumbers the line the file then ends on, so its `seq` still reaches the mark, hides the removal again. Taking the last stated `seq` rather than the largest is what keeps the cost there: against a maximum, one planted line anywhere in the remainder would have answered for the end. That is strictly narrower than the state this replaced, where changing a single character was enough, but it is not equivalent to authentication and is not claimed to be.

**A second route costs the same one line and needs no renumbering.** Delete the tail and end the file on a line declaring a `chain_version` this binary does not recognize and stating no `seq` that reads as an unsigned 64-bit integer. Verification halts there, nothing at or after the halt states an end, the comparison is skipped, and the verdict is exit 4 with the removal unreported. That has held since #470 for such a line with no `seq`; since [#556](https://github.com/yottayoshida/omamori/issues/556) it holds too for one whose `seq` is a string or a negative number, which through `1.2.0` counted as a torn line and let the walk reach the mark, so the same file reported exit 3. The skip is the forward-compatibility rule above rather than an oversight — comparing the mark against the halt point instead would report truncation on a genuine log a newer release had extended — and exit 4 is not silence: `report`/`doctor` are non-quiet, and the operator guidance under Forward-Unknown Chain Versions (treat exit 4 on the latest release as possible tampering) applies. The `.hwm` sidecar itself remains plaintext and directly overwritable by a same-user attacker — the structural limitation recorded in the Defense Boundary Matrix, unchanged here.

**Structural continuity across the halt (#470, second half).** When `audit verify` stops because a key is unavailable — an entry naming a `key_id` the keyring does not hold, or a key store it could not use at all — a break in the `seq`/`prev_hash` continuity between two readable chain entries that follow is reported as its own finding, separately from the authenticated `broken_at`, and no exit code moves. The comparison starts at the halting entry itself, against what the walk had authenticated up to it, and continues pair by pair through the region. Like the high-water-mark comparison above it needs no key; unlike it, the fields it compares are the ones an attacker would rewrite, so **the finding is one-sided**: a break is evidence someone left one behind, and a clean result asserts nothing at all.

What it declines to compare across, and why each would otherwise report a break on a store nobody touched: a torn line; a legacy-shaped line; a line from a `chain_version` this binary does not recognize, wherever it sits; a line missing `seq`, `prev_hash` or `entry_hash`, or stating one of them with the wrong JSON type; a line stating `seq: 0`, which is what the head of a chain carries, what a prune point carries (the retained entry behind it keeps its original number, and the authenticated path skips the same two checks there), and what an append through `1.1.0` wrote when it restarted a chain mid-file; and a line carrying a hash longer than 128 bytes, which bounds what a hostile line can make the scan hold. **Planting any one of those suppresses the comparison over that point** — one line, unlike the `seq` rule above, which was chosen precisely so a single insertion would not do. Rewriting the region consistently passes it too, and so does writing it in the unprotected shape (`entry_hash` set to the `NO_HMAC_SECRET` sentinel), where the linkage check compares one sentinel against another. What the scan catches is a removal nobody tidied up after.

**Out of scope: a halt at an unrecognized `chain_version`.** There, what `seq` and `prev_hash` mean is the thing this binary does not know, so comparing them would report breaks on genuine logs from a newer format — the forward compatibility the halt exists to provide. A run that could compare nothing says so; one that compared and found the region consistent says nothing at all, since that result asserts nothing. The count itself is on `VerifyResult` for a Rust caller.
