...
**Residual risk — the path shown on that prompt is sanitized against control characters only** ([#515](https://github.com/yottayoshida/omamori/issues/515)). An absolute `audit.path` reaches the prompt (and `omamori status`) inside a filesystem error message, and `config.toml` is attacker-controlled in this threat model. Characters `char::is_control` reports are replaced at the point the message is built, which stops cursor movement and line erasure from rewriting the warning being answered. Bidirectional formatting characters are not in that set — U+202A–U+202E, U+2066–U+2069, U+200E/U+200F — so a path can still be made to render in an order other than its bytes. The same coverage `provenance::sanitize` has applied to `parent_process` since #420; what is new is that one of the surfaces is a consent prompt.
