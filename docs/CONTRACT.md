...
| v1 | `1.0.6` | [#542](https://github.com/yottayoshida/omamori/issues/542): `doctor`'s Layer 1 compares each shim against the binary running the check, not only against `.integrity.json`. **Neither (b) nor (c) — a defect fix, with one verdict moving.** No guarantee's wording or scope changes, and the meaning of a documented exit code is unchanged: exit 2 has always meant warnings are present. Recorded here because a machine holding two genuine installs — a supported configuration, listed in SECURITY.md's v1.0 soak notes — reports a different verdict for a byte-identical filesystem. Layer 1 previously compared the shim symlinks only against `.integrity.json`, which `install`, `status --refresh` and `doctor --fix`'s baseline step each rewrite from current state, so a shim pointing at an older install agreed with its own record and Layer 1 reported full marks. Every surface that renders these `CheckItem`s moves together: `doctor`'s Layer 1 count and `doctor --json`'s `summary.layer1.pass` each drop by one per drifting shim (normally all five, since `install` writes them in one loop from a single source), `status`'s `Shims:` listing gains a `WARN` line per shim, and `setup`'s `[3/3] Verification` tally counts them. On a machine with nothing else warning or failing, `doctor`'s exit code moves 0 → 2 and `summary.protection_status` moves `ok` → `warn`; where something was already warning, only the counts move. The finding carries a manual remediation rather than an install repair, so `doctor --fix` reports it instead of re-linking shims an operator may have pinned with `install --source`. Nothing that previously passed Layer 1 for the right reason stops passing. See ADR-0011. No contract version bump. |
| v1 | `1.2.0` | [#465](https://github.com/yottayoshida/omamori/issues/465): `append()` picks up the chain at the log's last chain entry, reading past non-chain content within the last 64 MiB, instead of at the last JSON object within a 64 KB tail window. **Neither (b) nor (c) — a defect fix, with three behaviours moving.** No guarantee's wording or scope changes, and no documented exit code changes meaning. Recorded here because a byte-identical store can now be appended to differently. (1) A store whose tail is non-chain content behind an unrecognized-`chain_version` entry used to accept the next append — restarting at `seq 0`, because the window no longer held the entry or because a foreign JSON line read as "no chain" — and now refuses it, so under `[audit] strict = true` a guarded command that used to run now blocks: the block that entry already caused without the padding. (2) A store whose tail is a chain-shaped line omamori never writes (missing `seq`, empty `entry_hash`), or a genuine entry longer than 64 KB, used to get a second chain restarted at `seq 0` behind it — which `audit verify` then reported as broken — and now gets the next entry linked to the last real entry; `verify` reports the line as torn, or, for a full entry whose hash does not check out, as broken at that line exactly as before. (3) A store whose last 64 MiB hold no chain entry used to restart at `seq 0` and now refuses; that includes a single entry longer than 64 MiB and a pre-v0.7.0 log of more than 64 MiB of legacy entries. The limit bounds the time `hook-check` spends before it can print a deny (SECURITY.md → [Forward-Unknown Chain Versions](../SECURITY.md#forward-unknown-chain-versions-177)). G-2's tamper-evidence gains nothing and loses nothing: the append path still does not authenticate the entry it links to. The #542 row above is the precedent for logging a defect fix whose behaviour moves. See ADR-0014. No contract version bump. |
| v1 | `1.2.1` | [#556](https://github.com/yottayoshida/omamori/issues/556): an entry declaring a `chain_version` this build does not recognize is reported as one whatever else its line holds, and `append()`, `audit verify` and the prune scan decide that through one reader. **Neither (b) nor (c) — a defect fix restoring documented behaviour, with verdicts moving.** No guarantee's wording or scope changes, and no documented exit code changes meaning. Recorded here because byte-identical stores report differently. (1) A line declaring an unrecognized `chain_version` whose `seq` does not read as an unsigned 64-bit integer used to count as a torn line — `audit verify` exit 0, `report --json` `intact`, `doctor` quiet — while `append()` refused to write behind it; it now halts verification with exit 4, `unverifiable`, and `doctor`'s risk line, as SECURITY.md → [Forward-Unknown Chain Versions](../SECURITY.md#forward-unknown-chain-versions-177) has always said. Whatever that misreading let the walk reach behind the line is displaced exactly as an unrecognized-version entry with a readable `seq` has displaced it since #177 B1: a break (exit 1), an entry naming a key the keyring does not hold (exit 2), an entry written unprotected (#483, exit 2), the structural scan, and the high-water mark's bootstrap and update. (A key store that cannot be used at all halts before the walk, #506, and is unaffected.) (2) **The #470 row above holds only where a line at or after the halt states a `seq` that reads as an unsigned 64-bit integer.** A halted store with its tail removed and no such line reports exit 4, not exit 3 — since #470 for a line with no `seq`, and since this change for one whose `seq` is of another type, which through `1.2.0` reported exit 3 only because the line was misread as torn. An attacker gains nothing from this: one line with no `seq` already did the same, and SECURITY.md → [Truncation Detection Across a Halt](../SECURITY.md#truncation-detection-across-a-halt-470) now states the route. In the other direction, a genuine log from a newer release that changed `seq`'s type no longer risks a false exit 3 from comparing the mark against the older end. (3) A line that is not a JSON object is an event to no reader of the log. Every reader used to take a JSON array positionally, as a derived `Deserialize` does. `audit verify` now counts one as torn: a whole entry written as an array was verified like the object it spelled (removing it from the chain now reports a break, or truncation when it was the last entry), while two shapes move to a weaker exit code — an array whose `chain_version` position held an unrecognized version (`[999,7]`, or a whole future entry spelled positionally) reported exit 4, and a legacy-shaped array spliced after the chain began failed closed (exit 1). `audit show`, `report` and the prune scan no longer display or count either, so what they spelled reaches no surface. `append()` reads `[999]` past instead of refusing behind it. (4) The prune scan uses the same reader, so a prune point's `unverifiable` count moves with `audit verify`: a line naming `chain_version` twice is no longer counted, and an unrecognized-version line holding a number out of range (`1e400`) now is. No contract version bump. |
| v1 | `1.2.2` | [#564](https://github.com/yottayoshida/omamori/pull/564): stash-then-exec stashes the repository `git reset --hard` acts on — including `-C`, `--git-dir`, `--work-tree`, `GIT_DIR`, `GIT_WORK_TREE` — instead of the process cwd, and the git-aware and path-based downgrades no longer judge the cwd for such a reset. **Neither (b) nor (c) for G-1's redirect — a defect fix restoring documented behaviour**: `docs/how-it-works.md` already said reset --hard stashes first; for a reset pointed elsewhere the stash landed in the wrong repository and the target was left unstashed. **Plus one category (c) expansion**: a reset carrying a global option the stash cannot follow (`-c`, `--config-env`, `--bare`, `--namespace`, ...) is now blocked rather than run; a closed list of options that touch no repository still passes. Verdicts that move, all toward the documents: such a reset from a dirty cwd leaves the cwd untouched and stashes the target; from a clean cwd it is stashed, not downgraded to log-only; with an unfollowable option it is blocked. `git clean -x`/`-X`, a repository with `status.showUntrackedFiles=no`, and a path rule matching a git ref or option value no longer downgrade `git clean -f` or `git push --force` to log-only. No contract version bump. |
