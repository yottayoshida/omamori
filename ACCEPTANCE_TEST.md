...
assertion は `exit != 0` / `exit = 0` を中心に、stable な stderr substring を OR fallback で書く。実出力 prefix の集合は: `omamori hook:` (Layer 2 hook block、`src/engine/hook.rs`) / `omamori blocked` (rule-message path + sudo escalation、`src/rules.rs` + `src/engine/shim.rs`) / `omamori failed to move` (Trash failure → fail-close 経路、`src/actions.rs`、S-2 `/etc/fstab` の EPERM ケースなど) の 3 系統。

## 前提

> ⚠️ **release 候補 binary を local install してから acceptance test を始める**: 旧 binary (例: v0.10.2) が PATH に居る状態で AI agent session を起動すると、Fenced Block #4 (#240 effect 検証) や T-* 系の row が **AI session 自身の hook (旧 binary) で self-block** され完走できない。release 前 acceptance は必ず release 候補 binary に install 切替後、`omamori install --force` で hooks/shims を再生成し、Claude Code session を restart した上で開始する。

```bash
# (1) release 候補 binary を PATH に install (cargo source build からの場合)
cd /path/to/omamori-source && cargo install --path . --force

# (2) hooks / shims を新 binary 経由で再生成
omamori install --force

# (3) Claude Code session を restart
#    既存 session 内 hook script は古い binary path を embed している場合がある

# (4) 開始時 sanity check (このセクションを始める前に必ず通す)
omamori --version | grep -qE '0\.10\.12' && echo "v0.10.12 ready" || { echo "FAIL: install release-candidate binary first"; exit 1; }
...
block 系の stable substring: `omamori hook:` (Layer 2) / `omamori blocked` (rule-message + sudo escalation) / `omamori failed to move` (Trash failure 経路、S-2 等) の 3 系統 (実装 prefix 集合、L35 と同期)。複合は `∧` で連結。
...
> 実行: raw terminal + `CLAUDECODE=1`。`which rm` で shim 経路を先に確認。Layer 1 shim block は `ActionOutcome::Blocked/Failed` から `exit != 0`、message は `omamori blocked` (rule-message + sudo escalation) または `omamori failed to move` (Trash failure)。
