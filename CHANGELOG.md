...
- **`git reset --hard` now stashes the repository it resets, not the directory it was run from.** `git -C <dir> reset --hard`, `--git-dir` / `--work-tree`, and `GIT_DIR` / `GIT_WORK_TREE` all point the reset somewhere else, and stash-then-exec ran `git stash` in the process cwd regardless. Run from inside another repository, that moved the cwd repository's uncommitted and untracked work into its stash, where nothing said it had gone, and left the repository actually being reset unstashed, so its changes were lost. The "no changes, so log-only" downgrade judged the cwd too: a clean cwd let such a reset run with no stash at all. Found when cargo-deny, updating its advisory database with `git -C <db> reset --hard`, stashed a project directory's Cargo.lock change. The stash now carries the invocation's own `-C` / `--git-dir` / `--work-tree` options in order and inherits the same `GIT_*` variables as the reset; the downgrade is not applied when any global option or any of `GIT_DIR`, `GIT_WORK_TREE`, `GIT_INDEX_FILE`, `GIT_COMMON_DIR` is present.
  ...
  Behaviour that moves with it: a `git reset --hard` carrying a global option that can change where it lands and that omamori does not follow (`-c`, `--config-env`, `--bare`, `--namespace`, `--exec-path`, `--attr-source`, ...) is now blocked in an AI session, with break-glass as the way out; `--no-pager`, `-P`, the pathspec flags and the other options that touch no repository still pass. `git -C <dir> reset --hard` from a directory that is not a repository now stashes `<dir>` and runs, where it used to fail on the stash and refuse. For those `-C` / `--git-dir` / `--work-tree` forms, a target repository with no commit yet, or mid-merge with conflicts, fails on the stash and the reset is refused.
- **The path rules no longer read a git option's value, or `reset`'s arguments, as a path to delete.** With `[context]` configured, `git -C node_modules/pkg reset --hard` matched the built-in regenerable list through the `-C` value and was downgraded to log-only, so it ran with no stash; `-C ~/src/proj` matched the protected list and was blocked; `git reset --hard build` and `git push --force origin build` were downgraded because a ref shared a regenerable directory's name; and `git clean -fd -e node_modules` was downgraded through the `-e` value while deleting everything else untracked. For `git`, only `clean` now has path targets — the one git command a path confines — and only without a global option: the arguments after `clean`, skipping `-e` / `--exclude` values (`git clean -fd node_modules` is still judged by `node_modules`). The audit log's `target_count` is unchanged.
- **`git clean -f` is no longer downgraded when a repository hides untracked files from `git status`.** The check now asks for `--untracked-files=normal`, which overrides `status.showUntrackedFiles=no`; before, such a repository read as having nothing for `clean` to delete.
- **`omamori explain` reports the new block.** For a `git reset --hard` carrying a global option the stash cannot follow, Layer 1 said `stash-then-exec`; it now says `block` with the reason.
- **`git clean -x` / `-X` is no longer downgraded to log-only when there are no untracked files.** `git status --porcelain` does not list ignored files, so "nothing untracked" said nothing about the `.env` and build outputs these flags delete; the rule's block now stands.

### Docs
- **The README is now a front page: 407 lines to 80, 24 headings to 6, 6 tables to 1, no collapsed sections.** It keeps what decides whether to install omamori — a one-line description, the demo, install and one command to try, three things it does, and the Verifiable Claims table with the limits CI cannot check. Everything else moved unchanged to `docs/`: `how-it-works.md` (what it blocks, tool compatibility, defense layers, limitations), `configuration.md`, `cli.md`, `troubleshooting.md`, `verifying-claims.md` (the full "How these are checked"), and `field-notes.md`. Links into the old README sections from `SECURITY.md`, `docs/FAQ.md` and `docs/CONTRACT.md` point at the new locations. `verify-claims.sh`, `check-boundary-matrix.sh` and `check-invariants.sh` still check the README the same way; only comments in `verify-claims.sh` changed, to name where the moved text now lives.
...
- **`omamori doctor` points at its risk signals from the headline when they hold something to act on.** ([#474](https://github.com/yottayoshida/omamori/issues/474))
  ...
  The headline is computed from the integrity checks alone and the risk-signals section is display-only, so a store whose audit log could not be verified printed `Protection status: OK` at the top, `chain: cannot verify` twenty lines down, and exited 0. A reader who stopped at the first line was told the wrong thing.

  The headline's word is unchanged — `ACCEPTANCE_TEST.md`, `docs/evaluation-kit.md` and the evaluation issue template all assert a healthy run by matching `Protection status: OK`, and appending to it would make those assertions pass in both states. What is added is a line beneath it. It fires on the chain's verdict, the states that say the chain could not be checked, unprotected entries, a damaged keyring, an unusable high-water-mark, an unwritable audit log, and what a prune recorded about the range it removed (written only when that range held something unverifiable, and on a pruned log often the only signal left). It does **not** fire on the blocks omamori made or on unknown-tool fail-opens: those are omamori working, the README's own sample shows 42 blocks in seven days, and a headline note driven by them would never be off. The risk-signals section is read once and shared with the headline, so the two cannot disagree about a log that changed between two reads. **Exit codes are unchanged**: `doctor`'s three values are frozen by `docs/CONTRACT.md`, and #474 files the exit-code asymmetry as a separate question. `docs/CONTRACT.md` is not amended — no exit code, `chain_status`, `--json` field or headline word moves.

  Closing #474 also closes its other three items, each overtaken by a later change: the long `doctor` line (item 1) was split by [#483](https://github.com/yottayoshida/omamori/issues/483), which made every cannot-verify line delegate to `audit verify`; `report`'s missing remedy (item 2) rested on `doctor` inlining one, which that same change removed — for the two states item 2 names (a key not in the keyring, an unusable key store) both surfaces now point at `audit verify`, `report` doing it through `Suggested follow-ups`; and the `KeyringAnomaly` granularity mismatch (item 4) was settled by [#471](https://github.com/yottayoshida/omamori/issues/471), which gave `Unreadable` the full path.
- **`audit verify` keeps checking `seq` continuity and `prev_hash` linkage past a halt it stopped at for want of a key.** ([#470](https://github.com/yottayoshida/omamori/issues/470))

  A halt stops authentication. It used to stop these two checks as well, and they need no key: an entry naming a `key_id` the keyring does not hold — or a key store that could not be read at all — left every line after it counted and nothing else. Deleting a run of entries and renaming one `key_id` was therefore enough to put a removal out of view, which is the second half of what #470 describes; the first half (the high-water-mark comparison, which the same halt used to suppress) shipped in 1.0.0.

  The scan starts at the halting entry, against what the walk authenticated up to it, and continues pair by pair through the lines that follow. A break is reported as its own finding — by `audit verify` wherever a halted run reports (the exit-2 and exit-3 paths; the exit-4 path is the future-format halt, which is out of scope and stays silent), and by `doctor` in its risk signals — never as `broken_at`, and **no exit code moves**: the halt still decides those. `report --json` is unchanged (its eight fields are fixed by SEC-R2).

  **The finding is one-sided.** Nothing in that region is authenticated, so a break is evidence someone left one behind, while a clean result asserts nothing: the fields compared are the ones an attacker rewrites. The scan also declines to compare across a torn line, a legacy-shaped line, a field of the wrong type, a line stating `seq: 0` (a prune point, or a chain restarted mid-file by a release through 1.1.0) or a hash longer than 128 bytes — each of which would otherwise report a break on a store nobody touched, and each of which therefore suppresses the comparison over that point if planted. A halt at an unrecognized `chain_version` is out of scope, since the meaning of these fields there is exactly what this binary does not know. `docs/CONTRACT.md` is not amended: no verdict moves for a byte-identical store, which is what the rows in its revision log record.
...
- **A shim-only install no longer gets a Claude Code hook entry that points at a missing script.** With `omamori install` (without `--hooks`) on a machine where `~/.claude/settings.json` exists, the first guarded command merged an omamori entry into it naming `~/.omamori/hooks/claude-pretooluse.sh` — a file a shim-only install never creates. The shim's settings sync now does nothing while there is nothing at that path (a dangling symlink, or a file named `hooks` in the way, is still synced to as before), the same line its hook-script check already held: installing hooks is `omamori install --hooks`, `omamori setup` or `omamori doctor --fix`'s job. An entry an earlier release already added is not removed by upgrading: `omamori doctor` reports it (`script path missing`), and `omamori install --hooks` or `omamori doctor --fix` installs the script it names (for a `--base-dir` install whose entry was replaced, `omamori install --hooks --base-dir <dir>`); to stay shim-only, delete the entry from `settings.json`. The same check stops a second case: with hooks installed under `--base-dir`, a guarded command replaced that install's working entry with one naming the missing `~/.omamori/hooks/claude-pretooluse.sh` (the shim syncs against `~/.omamori` wherever its links live); that entry is now left in place. Found while tracing [#526](https://github.com/yottayoshida/omamori/issues/526).
- **`omamori doctor` no longer reports `[Staging] empty` for a staging directory it could not read.** A listing that stopped partway — a detached volume, a failing disk — ended quietly and was counted as complete, and a staging file whose stat failed was left out of the count; a directory that could not be opened, a symlink or a file in its place, and a `HOME` that does not resolve (reachable with `--base-dir`) all came out as empty. The section now says `[Staging] cannot read: …`. In `--json`, `summary.staging.status` is `"error"`, `file_count`, `total_bytes` and `oldest_days_ago` are `null` rather than `0`, and a new `reason` field names the case in one path-free word: `listing_stopped`, `unlistable`, `unreadable`, `symlink`, `not_directory`, `entry_unreadable` or `home_unresolved`. `"error"` was already the status when the config cannot be loaded; that case keeps its real counts and has no `reason`. A staging directory that does not exist yet is still reported as empty. ([#485](https://github.com/yottayoshida/omamori/issues/485))
- **The staging prune no longer applies its file-count cap to a partial listing.** The cap deletes the oldest files it saw; over a listing that stopped partway, or one with a staging file that could not be stat'ed, that could be a newer receipt while older ones went unseen. The count pass now waits for a complete listing; the age pass, which decides each file on its own date, still runs — so with `retention_days = 0` the directory can stay over its cap while the listing stays incomplete, which `doctor` reports. ([#485](https://github.com/yottayoshida/omamori/issues/485))
- **`hook-check --json-error` now records a blocked shell command in the audit chain, as text mode does.** It used to skip the row so that stderr stayed a single JSON object: the audit layer printed its own warnings (a key store it cannot use, a failed append), and a line of text ahead of the JSON would have broken it. The audit layer now hands those warnings back to its caller ([ADR-0013](docs/adr/0013-audit-warnings-are-returned-and-the-caller-prints-them.md)), and `--json-error` carries them in a new optional `warnings` array inside the object — with a healthy store and configuration it is absent except on the run whose append triggers the audit log's periodic prune, which adds that note. Structural blocks gain the same guarantee (their config-load and degraded-config warnings were printed ahead of the JSON), and so does a break-glass bypass that cannot be audited under `[audit] strict = true` — a block that printed text and no JSON at all. Text-mode output is unchanged. ([#494](https://github.com/yottayoshida/omamori/issues/494))
...
- **`doctor` and `status` no longer replace their exit code when the reader goes away.** ([#544](https://github.com/yottayoshida/omamori/issues/544))
  ...
  Measured on a healthy v1.0.5 install: `omamori doctor` exited 0, `omamori doctor | head -3` exited **101**. `doctor`'s exit code is a three-value contract — 0 healthy, 1 fail, 2 warn-only, each pinned by its own test — and 101 is none of them. Rust disables the default `SIGPIPE` disposition at startup, so a write to a closed pipe returns `EPIPE` instead of terminating the process, and `println!` panics on it. `| head`, `| grep -q`, quitting out of `| less`, or any script reading the first few lines was enough. `doctor` is the command the README tells operators to run and the one a monitoring script reads, so the error ran in the direction that matters: a false alarm on the surface built to answer "is the guard intact".

  Both commands now render through a sink that drops output once a write fails, and return a verdict that never consulted the output — `doctor`'s comes from the check list, `--fix`'s from what the repairs did. **This is the answer `audit show` has given since [#457](https://github.com/yottayoshida/omamori/issues/457)**, where a departed reader was ruled a normal way to read a long log rather than a failure; #544 extends it rather than introducing a second answer. Write failures are swallowed rather than propagated, because `doctor --fix` interleaves printing with repairs and a propagating error would abandon a repair half-done. All 102 stdout writes in the two files were converted, and a test scans both files to keep the conversion total.
...
**On the soak period v0.16.0 promised**: it named four things CI cannot replicate, and two of them happened. A real `audit.jsonl` spanning the `CHAIN_VERSION` 1→2 boundary exists and was verified with this build — 550,112 entries over four months, 275,545 `chain_version: 1` and 274,567 `chain_version: 2`, `exit 0, chain intact` in 1.5s — which is the audit-chain surface of the breaking-change policy checked against real data rather than a fixture. Homebrew and `cargo install` binaries have coexisted on the development machine throughout. The other two did **not** occur and are therefore unverified outside CI: retention pruning never ran (`retention_days` is unset, so the store keeps everything), and key rotation was never invoked on that store (every entry names `default`; no retired key files exist). Both are covered by tests, and #457's rotation work is extensive, but neither has been exercised against a real long-lived store.
...
- **BREAKING**: `audit::VerifyResult` gains `key_store_failure: Option<audit::KeyStoreFailure>`, `never_protected_entries: u64` and `hwm_compared: bool`; `audit::report::ChainStatus` gains an `Unprotected { entries }` variant; `audit::report::ReportAggregate` gains `key_store_failure` and `never_protected_entries`; `audit::KeyStoreFailure` is a new `#[non_exhaustive]` public type. Both structs already carry `#[non_exhaustive]`, so this adds fields rather than closing a door. Zero known reverse dependencies on crates.io, re-measured for this change. Both new `ReportAggregate` fields are `#[serde(skip)]` — `report --json` keeps exactly 8 fields per SEC-R2, and `KeyStoreFailure::reason` embeds the data directory's path, the same reason `ChainStatus::KeyringUnusable`'s does. ([#506](https://github.com/yottayoshida/omamori/issues/506), [#483](https://github.com/yottayoshida/omamori/issues/483))

  **One `report --json` value moves.** A store whose epoch record states no epoch reported `{"status":"keyring_unusable","kind":"directory_unreadable"}`, because both fatal keyring conditions arrived as a single `AuditError::KeyringUnusable` and the mapping could only pin one `kind` on it. That store now reports `kind: "epoch_record_unreadable"`. Its directory is perfectly listable and the old label said otherwise; carrying the error through the verifier would have carried the mislabel with it.

  Every other store reports what it reported before, deliberately. The `kind` values are unchanged, and each one's `ChainStatus` mapping is reproduced rather than rewritten — a given store's `chain_status` and `kind` are the same after this change as before it. What moved is only that the walk now continues far enough to *also* notice a missing tail.

- **BREAKING**: `audit::report::ChainStatus` gains an `Inaccessible { reason, kind }` variant, `audit::AuditError` gains `StoreInaccessible { kind, reason }`, and `audit::report::ReportAggregate` gains a public field `keyring_warnings: Vec<String>` and is now marked `#[non_exhaustive]`. Zero known reverse dependencies on crates.io, re-measured for this change. `reason` embeds the data directory's path on both new shapes and is **not** serialized, matching `KeyringUnusable`; `kind` is the path-free classification `--json` consumers branch on (`secret_symlink`, `log_symlink`, `rotation_interrupted`, `secret_unreadable`, `log_unreadable`, `log_missing`, `active_key_missing`, `log_lock`, `log_read`, `path_unresolved`, `unclassified`). `keyring_warnings` is `#[serde(skip)]` — `report --json` keeps exactly 8 fields per SEC-R2, same as `hwm_tampered`. Library-API only; `cargo install`/Homebrew binary users are unaffected. ([#471](https://github.com/yottayoshida/omamori/issues/471), [#487](https://github.com/yottayoshida/omamori/issues/487))
...
- `omamori audit hash-cwd` now reports an unreadable key directory instead of attributing it to a missing secret. The keyring for such a directory is empty rather than partially filled — registering the active key would mean naming it with the epoch the failed scan could not produce — and the anomaly is reported before that emptiness is acted on. Previously the investigator saw only "no audit path or HMAC secret available", which is false when the secret is readable and only the directory is not. ([#477](https://github.com/yottayoshida/omamori/issues/477))

- **The repair suggested for an unreadable key directory no longer names a cause.** Each of the four messages said to fix the *permissions*, which is wrong for the cause this release makes reachable: a listing that stops partway, produced by a detached volume or a failing disk, where a `chmod` fixes nothing. The cause is already in the message and comes from a single constructor, so the repair no longer repeats it. `audit verify` asks for the directory to be listable again; `audit key rotate` asks for listable **and writable**, since it renames and creates — a directory made merely readable still fails there; `doctor` points at `audit verify` instead of repeating the advice, which also brings its risk-signal line back in line with its neighbours. ([#477](https://github.com/yottayoshida/omamori/issues/477))

- The per-entry explanation for an entry written without a key no longer advises fixing the directory. That message is only ever printed once the directory is readable again — verification stops before reading any entry while it is not — so it now says the condition has already cleared, and that this entry stays unverifiable regardless.

- The `audit key rotate` refusal says "No key file was renamed or created" where it previously said "Nothing was changed". `audit-secret.lock` may exist by then; it holds no key material, but the stronger sentence was not true as written. Relatedly, "Old entries will still verify against the retired key file" now prints on success only — it was printed before the attempt, so it also greeted every refusal, where no retired key file exists.

- The warning printed when rotation cannot take the key-store lock now quotes what was observed rather than naming a cause, and names the symptom to expect. Four conditions reach it, and two of them — the lock path being a symlink or not a regular file — are omamori's own file-type guards firing on a planted path. The previous wording described a concurrent reader in all four cases, which filed those two as routine contention and left no way to connect a later tampering verdict back to this warning.

- The warning printed when the writer cannot resolve a key epoch no longer says that fixing the condition restores protection. It protects later entries; the ones written meanwhile carry no HMAC and stay unverifiable, which is what `audit verify` says about them.

- **`omamori audit verify` exit code 2 now also covers "an entry names a key that is not in the keyring".** Exit 2 already meant *cannot verify* (secret unavailable, file missing, I/O error, legacy-only log); a missing key file is the same class. Deliberately not exit 1, which asserts tampering, and not exit 4, whose message instructs the reader to upgrade omamori — useless advice for a key that was renamed or deleted. `omamori report --json` reports `"status": "key_unavailable"` and `omamori doctor` says `chain: cannot verify` rather than `chain: broken`. No existing exit code changes meaning.

  The message this prints is deliberately not an assurance. `key_id` is an ordinary field of `audit.jsonl`, so editing one string is enough to reach this state — no key material, no re-signing — and an unconditional "this is not evidence of tampering" would be an exoneration written by whoever edited the log. omamori now says only what the evidence supports: an id no omamori build emits is reported as altered, an entry omamori itself wrote unprotected is reported as such, and a well-formed epoch id names the file to look for while stating plainly that an edit produces the same state. Exit 2 also now says what happened to the tail-truncation check, matching what exit 4 already said — the shared wording was rewritten by [#470](https://github.com/yottayoshida/omamori/issues/470) in this same release, since the check is no longer suspended by a halt. ([#457](https://github.com/yottayoshida/omamori/issues/457))
- **BREAKING**: `audit::report::ChainStatus` gains two variants — `KeyUnavailable { at_seq, key_id }` (an entry names a key the keyring does not hold) and `KeyringUnusable { reason, kind }` (the key directory itself could not be listed, so no entry can be checked against the key it names; `reason` is human-facing and **not** serialized, because it embeds the data directory's absolute path, while `kind` is the path-free classification `--json` consumers should branch on) — and is now marked `#[non_exhaustive]` (zero known reverse dependencies on crates.io — same check and same precedent as `AuditEvent` in #177 B2 and `VerifyResult` in #177 B3). The two are deliberately distinct from each other and from `Unavailable`: `Unavailable` also covers "audit is off" and "there is no log", which `doctor` treats as quiet, and folding an actionable fault into it would remove it from the risk signals. Library-API only; `cargo install`/Homebrew binary users are unaffected. ([#457](https://github.com/yottayoshida/omamori/issues/457))
- **BREAKING**: `audit::AuditError` gains a `KeyringUnusable(String)` variant and is now marked `#[non_exhaustive]`. Same rationale and same reverse-dependency check as `ChainStatus` above. Kept apart from `SecretUnavailable` because the active key is often perfectly readable in this state — what failed is listing the directory to learn which epochs exist. ([#457](https://github.com/yottayoshida/omamori/issues/457))
- `audit::VerifyResult` gains `key_unavailable_at: Option<u64>`, `key_unavailable_id: Option<String>`, `key_unavailable_kind: Option<KeyUnavailableKind>` (whether the entry was written unprotected by omamori itself, names an id no writer emits, or names an epoch whose key file is absent — the verifier's remedy text is derived from this rather than assumed), and `keyring_warnings: Vec<String>` (non-fatal keyring problems — a truncated ring, or a file shaped like a retired key that could not be read — which `omamori audit verify` now prints for every verdict). Not breaking: the type is already `#[non_exhaustive]` with `Default`. ([#457](https://github.com/yottayoshida/omamori/issues/457))
- The keyring loads at most 256 **retired** keys (plus the active key and its `"default"` alias), keeping the **highest** indices, and reports the truncation rather than verifying silently against a partial set. Previously the loop had no bound at all. 256 is above any plausible legitimate schedule (weekly rotation for two years is 104) and a measured 200-key ring costs ~26 ms. ([#457](https://github.com/yottayoshida/omamori/issues/457))
- **New file in the audit data directory**: `audit-secret.lock`, an empty file used only as an `flock` target. Resolving the active key and rotating it each involve more than one filesystem observation, and a rotation landing between them would pair one epoch's bytes with another epoch's label — the defect this release exists to remove, through a narrower window. The lock serializes the two. It holds no data, is created `0600`, and is never treated as a retired key (the name does not end in `.retired`). Neither creating it nor acquiring it is allowed to be fatal: if either fails the operation proceeds unlocked, because refusing to record an audit event is worse than a rare mislabel. ([#457](https://github.com/yottayoshida/omamori/issues/457))
- `omamori audit hash-cwd` warns when the keyring is truncated or partly unreadable. A shorter candidate list with no indication that the matching key was never tried is a forensic false negative. ([#457](https://github.com/yottayoshida/omamori/issues/457))

### Fixed

- **An unreadable key store no longer switches off the tail-truncation check.** ([#506](https://github.com/yottayoshida/omamori/issues/506))

  `audit verify` compares the chain's end against the high-water-mark to detect a removed tail, and that comparison needs no key. Five states returned before it could run: a symlink planted on the secret path, a key directory that cannot be listed, an epoch record that states no epoch, a rotation that stopped halfway, and an active key missing from a store that already holds entries.

  Measured on `78b27ff`, one store seeded and its tail deleted: exit 3 with the keys intact, exit 2 with the active key renamed, exit 2 with the key directory at mode 0300 — and neither of the last two said one word about the deletion. Making the key material unreadable *was* how a tail got deleted quietly. It is the substitution [#470](https://github.com/yottayoshida/omamori/issues/470) closed for an edited `key_id`, reached through a different door.

  The failure is now recorded on the result and the walk continues. It stays terminal — `halted()` includes it, so every line is tallied rather than trusted and the mark is never written from such a run — and what the walk buys is the checks that never needed a key.

  **Two new false statements had to be avoided on the way, both found by review before any code was written.** Continuing past an unusable key store means every entry reaches the classifier that explains an unresolvable `key_id` — whose two inputs are then spoiled by the same fault, since `highest_known_epoch()` returns 0 on an empty ring. A store nobody had touched would have been told "Either this entry's key_id has been altered, or retired key files were deleted". And a store with an empty log and an unlistable key directory walks zero lines, halts at no entry and breaks nowhere, so `chain_status` would have landed on `Intact` — where `doctor` prints nothing at all, for a store it describes today. Both are pinned by tests that fail without the guard.

  A store-level failure travels **beside** `chain_status` rather than inside it when the two compete. `ChainStatus` holds one value, and exactly one other finding can be true at the same time: a truncated tail. Truncation takes the slot, because it is the finding that was being hidden, and the key failure is reported next to it — on `doctor`, on `report`, and in `audit verify`'s own output.

  One further reassurance was withdrawn rather than left standing: the halted note ended by telling the operator that "the chain does reach the high-water-mark" whenever the sidecar was neither missing nor tampered. That inference held while halting could only happen at an entry. A store whose keys are unusable halts before the first line, so a log with nothing readable in it reached the same note having compared nothing — `hwm_compared` now separates "compared, and it was long enough" from "there was no end to compare".

- **An entry omamori wrote unprotected no longer pins the store at cannot-verify permanently.** ([#483](https://github.com/yottayoshida/omamori/issues/483))

  While the key directory cannot be listed, appends still happen: they carry `key_id: "unresolved"` and no HMAC. That is deliberate — five of the eleven production append sites have no other durable trace, `SECURITY.md`'s forensic semantics state that a missing row implies Allow, and `[audit] strict = true` is what refuses the command rather than dropping the record.

  Verification then *halted* on such an entry, and ADR-0007 forbids rewriting entries. So one line written during a permissions blip held `audit verify` at exit 2 for good — including long after the permissions were fixed. Nothing the operator could do cleared it.

  The entry is now walked past on **two** pieces of evidence that have to agree: the id no keyring can hold, and the no-key sentinel in `entry_hash`. One field alone would not do — `key_id` is an ordinary field of the log, so relabelling an authenticated entry would slide it out of `broken` into a state that no longer halts, which is the exit-2 exploit [#457](https://github.com/yottayoshida/omamori/issues/457) closed, re-entered by a new door. Requiring both means an attacker has to destroy the entry's own hash, and the next line's `prev_hash` then stops matching, so the chain breaks at the line after.

  Everything that needs no key still runs on these entries: `seq` continuity, the `prev_hash` link, and the prune-bind — whose key belongs to the prune point and is in hand. `entry_hash` is not compared, because there is nothing to compare it against. What does **not** advance is `last_verified_seq` and `chain_entries`: the first is what the high-water-mark is written from, and the second is the count this command prints as verified. A prune point is excluded outright — its own key is what the next entry's bind is checked with, and there is none to record.

  Unprotected entries are counted, and the count holds `audit verify` at exit 2. Without it a log made only of them reaches "no entries to verify" and exits **0** — a clean bill of health for a log nothing in it could be checked against, and a hole that exists in neither this change nor [#506](https://github.com/yottayoshida/omamori/issues/506) alone.

  **The final line of a log is not covered and this is accepted.** Nothing follows it to hold a `prev_hash`, so forging both fields there is undetectable. Holding the command at exit 2 whenever any unprotected entry is present is what bounds the consequence.

  **Three things review caught that the first version got wrong, all of them measured before being fixed.**

  The append path advances the high-water-mark for *every* entry it writes, unprotected ones included. The verifier deliberately does not advance `last_verified_seq` past an unprotected entry — that value is what a later run's mark gets written from — and then compared the mark against it. A store whose last entry was written while the keys were unreadable reported `exit 3, audit log tail may have been truncated` on a log nothing had been removed from, printing "chain intact" one line above it. That trades a permanent exit 2 for a permanent false accusation, which is the worse of the two. The comparison end and the write end are now separate values: the comparison uses the last seq walked, the write still uses the last seq authenticated, and a run that walked past an unprotected entry writes no mark at all.

  `report --json` called such a store `intact`. The chain's links genuinely are intact, which was the argument — but a consumer branching on that value concludes every entry was checked, while `audit verify` exits 2 on the same store, and before this change those entries reached the JSON as `key_unavailable`. `ChainStatus` gains `Unprotected { entries }` rather than leaving a false green on the machine-readable surface.

  The head of the log skipped every check. Its genesis anchor cannot be *computed* without the key, but what omamori writes there is a constant — `seq: 0` and the no-key sentinel in `prev_hash` — and both are comparable without one. Without that, replacing a whole log with unprotected lines carrying any position and any anchor landed on the coverage complaint instead of on a broken chain.

  Two operator-facing sentences were rewritten to match what can actually be observed. `audit verify`'s never-protected explanation used to state that omamori wrote the entry itself and that the condition had cleared — an acquittal derived from two fields an attacker can write. It now says the fields disagree and to treat it as possible tampering, which is the only state that still reaches it. And `doctor`'s "restore the retired key file" advice, right for one of the three conditions behind that status and wrong for the other two, now names the cause and delegates the repair to `audit verify`, like every sibling line in that list.

- **`break-glass` no longer promises audit logging before it has checked whether auditing runs.** ([#492](https://github.com/yottayoshida/omamori/issues/492))

  The confirmation prompt printed "All bypassed executions will be logged to the audit chain." fifteen lines before the first `load_config`, and so before anything had looked. With `[audit] enabled = false` nothing was logged and the sentence that obtained consent was simply untrue. The same held for an audit path that does not resolve, which `AuditLogger::from_config` refuses identically — a predicate reading only `enabled` would have kept printing the reassuring sentence there.

  The prompt now says one of six things, because "recorded", "verifiable afterwards" and "allowed to run at all" are three separate promises and the operator is consenting to all of them. Auditing off, or a path that will not resolve: nothing is recorded, and the path error is named so it can be acted on. A resolved path whose log cannot be read: the append is attempted and fails. A healthy store: the original sentence, unchanged, byte for byte. A degraded key store: entries are written but carry no signature, and restoring the key later does not repair them (ADR-0007 forbids rewriting an entry). A config that could not be read at all: this command cannot tell, said plainly rather than borrowing either verdict.

  **Under `[audit] strict = true` the third of those is a refusal, not an omission.** A failed append returns exit 2 from the hook layer, so the bypass the operator is authorising does not run. Reporting it as "will still run, unrecorded" would have been a second false promise of exactly the kind this issue is about — and the state is reachable: a symlink planted on `audit.jsonl` produces it, which is one of the cases [#471](https://github.com/yottayoshida/omamori/issues/471) was about.

  Telling those last two apart needs more than the flags already on `AuditSummary`. `enabled` does not answer it — `AuditLogger::from_config` also refuses when the audit path does not resolve, and `enabled` is `true` there. Neither does `path_error`, which is `Some` in **both** of those unrelated states. `AuditSummary` gains `logger_available`, mirroring `from_config`'s own predicate, so "nothing will be written" and "the write will fail" stop sharing a sentence.

  The missing-active-key case is held apart from the other degraded ones, because it is the only state whose outcome is genuinely uncertain — the next append tries to mint a key and often succeeds, protecting the very entry being asked about. `UnprotectedReason::summary`'s own doc records that measurement; a single "cannot be verified later" sentence would have stated the opposite of what usually happens there. The unreachable `None` arm is attached to the cautious sentence rather than that one, so a state added later to this `#[non_exhaustive]` type inherits caution instead of reassurance.

  The question is asked through `audit_summary`, not `AuditLogger::from_config`. The latter runs `load_signing_key`, which **creates a key file** when none is there: a write, performed while asking a question, before the operator has agreed to anything. `audit_summary` has always been the read-only form and says so in its doc, but nothing measured it — a test now pins that the store's directory listing is still empty afterwards, with the writer's path on the identical directory as the control.

  One clause that is true only at the moment it is printed: `load_config` runs once before the prompt and again after the answer, so a config edited during the operator's keystrokes is described by the earlier read. That is inherent to checking before asking, and strictly better than the unconditional sentence it replaces.

  **Residual — the check asks whether the log can be read, not written.** A log that opens for reading and refuses writing (mode `0400`, a read-only mount, an `audit.path` pointed at a file this user cannot write) leaves `path_error` empty, so the reassuring sentence is still printed while every append fails. This is narrower than what it replaces — the old sentence was unconditional in every state — but it is the same class as the defect being closed, and it is reachable under this change's own threat model. Tracked separately rather than widened into this change: a writability probe is not a read, and deciding what it may create is its own question.

- **The two key-store warnings #457 added now pass through the throttle and the AI-disclosure gate they were bypassing.** ([#473](https://github.com/yottayoshida/omamori/issues/473))

  Scoped deliberately: `load_signing_key_locked` reaches four other unconditional printers — the occupied-secret-path message and the epoch-advance failure — and review measured that both also repeat per command in states nothing on disk resolves. They are the same class and are tracked separately rather than swept in here, so the headline says which warnings this covers.

  `load_signing_key` runs from `AuditLogger::from_config`, which the PATH shim builds on every guarded command. The two warnings [#457](https://github.com/yottayoshida/omamori/issues/457) added printed from there unconditionally, so a key directory that stayed unreadable produced them once per command — and this repository already had both mechanisms for exactly that: the 300-second stderr sentinel from [#359](https://github.com/yottayoshida/omamori/issues/359), and SEC-R5's rule against printing literal repair commands into a session an AI is reading.
  ...
  **The disclosure gate withholds the repair and nothing else**, and only one of the four reasons has one to withhold. The condition is stated in every case — suppressing the observation would hide a degraded key store from its reader, which inverts what SEC-R5 is for.
  ...
  The interrupted-rotation warning is throttled but **not** gated, and the issue's own description is why that had to be checked rather than assumed: it called that warning "a recovery procedure", and reading the whole of what its three shapes produce, there is no procedure in it. The parts are an observation, a consequence for entries already written, and one prohibition — do not copy a `.retired` file over `audit-secret`. Withholding a prohibition from the reader most likely to perform the action it forbids would remove the sentence from the audience it was written for, and that action destroys the key its own newer entries were signed with.

- **SECURITY.md now states the two rules governing operator warnings, and the four residual risks in them.** ([#518](https://github.com/yottayoshida/omamori/issues/518), [#519](https://github.com/yottayoshida/omamori/issues/519), [#520](https://github.com/yottayoshida/omamori/issues/520), [#521](https://github.com/yottayoshida/omamori/issues/521))

  Neither rule was written down. Throttling appeared only as one clause inside the forward-unknown-`chain_version` section, describing a Layer 1 / Layer 2 asymmetry for one warning; SEC-R5 — withhold literal repair commands from an AI session — appeared **nowhere**, existing only in code and tests. Recording the four residuals `#473`'s review found meant first saying what they are residuals of.

  The rules as written also state the two boundaries that are easy to lose: the sentinel must stay out of the audit data directory, because every warning it gates fires when that directory is unreadable; and a prohibition is never withheld, because "do not copy a `.retired` file over `audit-secret`" has to reach the reader most likely to do it.

- **SECURITY.md records the two residual risks the break-glass prompt ships with.** ([#514](https://github.com/yottayoshida/omamori/issues/514), [#515](https://github.com/yottayoshida/omamori/issues/515))

  Both were found by review while closing #492 and neither is fixed. The prompt's check asks whether the audit log can be *read*, so a log that opens for reading and refuses writing still gets the reassuring sentence — and under `[audit] strict = true` the bypass is then refused rather than run unrecorded, the opposite of what was said. And the path shown on that prompt is sanitized against control characters but not bidirectional overrides, so it can render in an order other than its bytes. Written beside the strict-mode paragraph they belong to rather than in a section of their own, since that paragraph already describes the enforcement the first one contradicts.

- **Control characters in an absolute `audit.path` no longer reach the terminal.** ([#492](https://github.com/yottayoshida/omamori/issues/492))

  `AuditConfig::validate` strips them only on the relative-path branch — where it discards the override anyway — so an absolute path went through verbatim into `AuditSummary::path_error`, which `status` prints and which `break-glass` now prints two lines above its consent prompt. `ESC[2A` plus `ESC[K` is cursor-up-two and erase-line: enough to overwrite the warning the operator is answering. Sanitized in `audit_summary` where the string is built, so both display sites are covered, using the same substitution `provenance::sanitize` applies to `parent_process`. `config.toml` is attacker-controlled by that function's own comment, so this is the same threat model, not a new one.

- **`doctor` and `report` no longer call a broken audit store healthy.** ([#471](https://github.com/yottayoshida/omamori/issues/471) items 2 and 3, [#487](https://github.com/yottayoshida/omamori/issues/487))

  `aggregate_report` mapped every `verify_chain` failure it did not name to `ChainStatus::Unavailable`, and `needs_attention()` treats `Unavailable` as quiet — correctly, because it also means "auditing is off" and "there is no log yet". Everything else that shared the bucket inherited that silence: a symlink planted on the audit log, a symlink on the key, a key rotation that stopped between filing the old key and creating its replacement, an unreadable key, an unresolvable `HOME`. `verify` reported each of them; `doctor` said `Last 30 days: quiet` and `report` said `Audit log: unavailable`.

  Rather than lifting the faults out one at a time, **the default is inverted**. Every error `verify_chain` can return was enumerated and given a destination; the two that genuinely mean "there is nothing to check yet" are named and stay quiet, and everything else — including any failure added later — is loud. For a tool whose product is trustworthy reporting, that is the direction the default belongs in.

  The classification happens where the information is: an `io::Error` alone cannot say whether it came from the log or the key, so `verify_chain` labels it at the call site that still knows. `kind` is path-free and stable for `--json` consumers; `reason` carries the directory and stays out of the serialized form, as `KeyringUnusable` already did. Exit codes do not move — `verify` said cannot-verify for all of these already, and still does.

  `keyring_warnings` also reaches `doctor` now. `verify_chain` had built them since [#457](https://github.com/yottayoshida/omamori/issues/457) and nothing read them, so a store with one damaged retired key printed a warning from `verify` while `doctor` said quiet and `report` said intact — and the chain genuinely *is* intact there, which is why this travels beside `chain_status` rather than moving it, the way `hwm_tampered` already does. `KeyringAnomaly::Unreadable` now names the full path rather than the bare file name, matching its `DirectoryUnreadable` sibling.

  **Two states nearly stayed in the quiet bucket by name alone.** The two quiet errors were called "there is no log yet" and "the first key has not been minted", and neither checked. Review measured both: deleting `audit.jsonl` outright left `doctor` saying `quiet`, and so did deleting the active key of a store that already held entries — the second while `status` was warning about the same store. Quiet now requires evidence that nothing was ever written, which is on disk and costs two stats: `audit.jsonl.hwm` is created by the first append and is not removed with the log, so a sidecar beside an absent log says the log existed. The second case matters beyond silence — the next append mints a *different* secret under the same id, after which the existing entries read as tampered permanently.

  The fixture that was supposed to cover the second one was named `fresh-store-no-key` and appended before removing the key, so it pinned the hole as correct. Renaming it to what it actually built is what made the question visible.

  Worth recording how the inversion nearly shipped unmeasured: a falsification probe that reverted the catch-all to `Unavailable` took **no test down**. Every loud state reachable through `verify_chain` needs a store broken a particular way, so each fixture lands on a named arm and the default itself is only reachable by a variant `verify_chain` no longer produces. The mapping is now a function that can be handed an error directly, and the probe fails against it.
...
- **A verification halt no longer hides a deleted tail.** ([#470](https://github.com/yottayoshida/omamori/issues/470), the high-water-mark half)
  ...
  `omamori audit verify` stops authenticating at an entry it cannot check — one declaring an unrecognized `chain_version`, or naming a `key_id` the keyring does not hold. It also used to stop comparing the chain's end against the high-water-mark, which needs no key and says nothing about authentication. That made a two-step attack free: rewrite one entry's `key_id` to an epoch this store has no file for — an ordinary text field, no key material, every hash on the line untouched — then delete as much of the tail as you like.

  Measured on v0.16.0 before the fix: exit 2, "cannot verify from entry #1", and not one word about the removal. Deleting the same lines *without* the relabel reported exit 3 and named the truncation. Editing one string was enough to swap the verdict, and [#457](https://github.com/yottayoshida/omamori/issues/457) had recently made that string a far cheaper trigger than forging a `chain_version`, which was the only route the original analysis considered.

  The comparison and the write are now gated separately, by what each can honestly use. The **comparison** runs whatever halted, against the last `seq` *stated* by a line at or after the halting line — that line included, and unverified by definition. Only a stated value counts, and it is the last one rather than the largest: a line that omits `seq` is reported at the position it should have occupied, which is one past the last verified entry and would compare the mark against the halt point instead of the file; and a maximum would let one line planted anywhere in the remainder answer for the end. Both were caught in review, and both are the same failure the halt gate originally existed to prevent, arriving by a different door. The **write** still comes from the last authenticated `seq` and still only from a run that halted nowhere; that half of the original judgement is load-bearing, since the mark is what every later run compares against. A halted run therefore reports a missing or unreadable mark without repairing either, and the messages say so rather than claiming a reset that did not happen.

  When truncation is found it outranks the halt on **all three surfaces that render a verdict**: `audit verify` exits 3 rather than 2 or 4, `report --json` reports `truncated`, and `doctor` prints the truncated line. Swapping one verdict for another was the point of provoking the halt, so a fix that reached only the exit code would have left the swap working for every `--json` consumer. Exit 3 is a new value for a byte-identical file, so it is recorded in CONTRACT.md's revision log.

  Two claims that stopped being true were corrected rather than left standing: SECURITY.md's Defense Boundary Matrix said truncation detection was "also suspended" during a halt, and both halted CLI messages ended on "Tail-truncation detection is suspended while this entry is present." — a sentence that told an attacker exactly what one edited field bought.

  **Residual risk, stated rather than closed**: the compared end is not authenticated, so an attacker who deletes the tail *and* renumbers the line the file then ends on hides the removal again. That is strictly narrower than changing a single character, and it is not equivalent to authentication. **[#470](https://github.com/yottayoshida/omamori/issues/470) stayed open** for the rest of what it proposes — `seq` continuity and `prev_hash` linkage across the halted region — which this release deliberately did not do: in a region the binary cannot authenticate, an attacker writes those fields too, so passing the check would assert nothing, and a future `chain_version` that redefines them would make it fail on genuine logs. (Superseded: see the entry at the top of this file. The first reason turned out to apply equally to the high-water-mark comparison this release *did* keep running, and the second only to a halt at an unrecognized `chain_version`, which is where the scan still does not go.)
...
- **A `protected_paths` entry of two or more components no longer stops applying when the working directory cannot be resolved.** ([#460](https://github.com/yottayoshida/omamori/issues/460))
  ...
  Escalation now happens where the information is missing, rather than a decision being made without it — and only there. The config has to carry an entry whose window is wider than one component, and the target has to be relative. **Absolute patterns are not exempt**: a first draft excluded them on the grounds that an absolute *target* never consults a base, which is true and beside the point — review caught that the dependency runs through the target's resolution. `protected_paths = ["/tmp/proj/secret"]` with a real directory of `/tmp/proj` and a relative target `secret` matches; against the synthetic root the target resolves to `/secret`, which cannot contain that window. Excluding absolute patterns re-opened, inside the exclusion, the hole this change closes. `.` and `..` do contribute nothing — normalisation drops the first and pops the second, so neither can appear in a resolved path — which keeps `protected_paths = ["."]` at one component, the config [#458](https://github.com/yottayoshida/omamori/issues/458) pinned as writable-but-never-matching.

  Measured against a `2ceade2` build with an unlinked working directory: a two-component config left the rule's own action in place where this build blocks and names the reason; a single-component config produced byte-identical output on both.

  `process_base_or_root` is gone. It applied the fallback unconditionally, which is only sound for patterns that need no base, so the choice moved next to the check that decides whether it is sound for the config in hand — callers now pass `process_base()`'s `Option` straight through. `evaluate_context` takes `Option<&Path>` accordingly (a signature change to a public function; crates.io reverse dependencies were checked and are zero). `explain` reports an unresolved base as such instead of printing `/`, and its `--json` field is `null` rather than `"/"`, so a consumer can tell "resolved to the root" from "there was no base".
...
- **`omamori audit key rotate` no longer files the current key under a guessed epoch when it cannot read the key directory.** It refuses instead — before any key file is renamed or created — and exits 1 with the directory named and every key left where it was. (The best-effort `audit-secret.lock` may be created first; it holds no key material and is never read as a retired key.) ([#477](https://github.com/yottayoshida/omamori/issues/477), [#482](https://github.com/yottayoshida/omamori/issues/482))
  ...
  The listing is now a value that either *is* a listing or says why it is not, and the number rotation needs is reachable only through the first. Every other reader of that scan already failed closed on it; rotation was the one consumer that took the number without asking, and the only one that changes the store. **A listing that stops partway now counts as no listing**: an error mid-enumeration ends the iteration in `std`, so skipping the failed entry did not drop one name — it dropped the entire unread remainder, silently, as a result that looks complete. For a key directory that is precisely where the highest epochs sort.

  Two smaller defects on the same three lines went with it. Rotation was flattening `read_secret`'s error, so a symlinked or non-regular secret reported as "no audit secret found — nothing to rotate" when the secret was right there; the scan now runs first, which also makes a mode-000 directory diagnose as unreadable rather than as missing. And rotation now says so when it could not take the key-store lock — non-fatal, since an unheld lock is a race window rather than a certainty, but no longer silent on the one operation whose exclusivity is a correctness requirement.

- **After upgrading, run `omamori audit verify` once.** It is the only way to learn whether your
  store was affected, and this release changes the answer in both directions: it stops reporting
  tampering on logs that were fine (below), and it stops reporting `intact` on entries it had not
  actually authenticated (see Security). Neither shows up until you run it.

- **`omamori audit key rotate` no longer breaks `omamori audit verify`.** Rotating the audit HMAC key even once made every subsequent `verify` return exit 1 `chain broken at entry #0`, permanently, with no way to recover — `ADR-0007` forbids rewriting existing entries, no `unrotate` exists, and the only remedy a user could reach unaided was deleting `audit.jsonl`. This shipped in v0.8.0 (2026-04-11) together with a changelog line claiming "Multi-key verification in `verify_chain()` — old entries verify against retired key"; **that statement was never true, across eight minor versions.** ([#457](https://github.com/yottayoshida/omamori/issues/457))

  **If you have a log in this state, it needs no re-audit.** The chain's cryptographic tamper-evidence (`docs/CONTRACT.md` G-2) was never broken — the failure was a *false positive*, not a missed detection. The verifier was recomputing the chain's genesis anchor with whichever key was currently active instead of the key the chain actually started with, so it rejected a first entry that was, and still is, authentic. Verified by measurement: on a rotated chain the fixed binary reports `intact`, and reports `chain broken` again the moment any entry in that same chain is altered — it distinguishes the two cases, where the old binary reported the same failure for both. Upgrading is sufficient; no `audit.jsonl` bytes are read or written differently, and nothing on disk is migrated.

  Three independent defects sat on this path, all instances of one shape — *a value signed with one key while labelled as, or checked against, another*:
  - The `genesis_hash` / `prune_genesis_hash` anchors were computed from the active secret rather than from the key each anchored entry names. This alone fires the instant a rotation happens, with no further activity required.
  - `build_prune_point` stamped `key_id: "default"` unconditionally while signing with the active key. After a rotation `"default"` names the *retired* key, so a prune point written post-rotation failed its own hash check. (Requires `[audit] retention_days > 0`, which is not the default.)
  - The prune-bind `target_hash` was written with the key active at prune time but recomputed by the verifier with the key of the *first retained entry* — a different epoch. This one survives fixing the previous two, and shows up at the first retained entry rather than at the prune point.

  A fourth instance was found in `AuditLogger::from_config`, which resolved the secret and the `key_id` through two independent directory reads; a rotation landing between them would mint one entry signed with the old key but labelled with the new id — unverifiable forever, and not repairable from the verifier side. Both are now one value (`SigningKey`).

- **A stray file next to the audit key no longer breaks verification.** Retired keys were counted by name match without parsing the index, so `audit-secret.bak.retired` — a name an operator naturally produces, since the rotation command prints the retired key's path — was counted as a rotation. Every entry written afterwards was labelled `key-2` while still signed by the same key, and the next real rotation skipped to `.2.retired`, leaving the sequential keyring probe to stop at the missing `.1` and lose the first epoch entirely. Suffixes that are not canonical decimal numbers are now ignored. ([#457](https://github.com/yottayoshida/omamori/issues/457))

- **A gap in the retired-key numbering no longer destroys a key.** With `.1.retired` and `.3.retired` present and `.2` missing, count-based numbering pointed the next rotation at `.3` and `fs::rename` overwrote it silently — the only copy of that epoch's key, gone, with every entry from it permanently unverifiable and nothing left on disk to say why. Numbering now derives from the highest index present, and rotation refuses to overwrite an existing retired key. ([#457](https://github.com/yottayoshida/omamori/issues/457))

- **A case-variant filename no longer breaks an unrotated store.** The keyring built `audit-secret.{n}.retired` and opened it, which on a case-insensitive filesystem (APFS default) succeeds for a file named `audit-secret.1.RETIRED` — a name the counting logic never saw. One such file made a host that had never rotated report `chain broken at #0`. The key directory is now enumerated with `read_dir` and names are matched as they are. ([#457](https://github.com/yottayoshida/omamori/issues/457))

### Security

- **Closed a false negative in `omamori audit verify`.** An entry naming a `key_id` absent from the keyring used to fall back to checking it against the *active* key. When that happened to be the signing key — the common case, since a stray `.retired` file shifts labels without changing which key signs — the entry passed, and the verifier reported `chain intact` about an entry it had not authenticated under the key that entry named. Measured against v0.16.0: exit 0 on a log containing such an entry. Verification now stops and reports it (exit 2, see Changed). ([#457](https://github.com/yottayoshida/omamori/issues/457))
- **A FIFO, directory or symlink planted at any file omamori reads no longer hangs it.** `open(O_RDONLY)` on a FIFO blocks until someone writes to it, which stalled not just `verify` but every hook-driven audit append — omamori stopped protecting anything, from one `mkfifo` in the data directory. Every read now goes through one helper (`atomic_file::open_read_regular`) that passes `O_NOFOLLOW | O_NONBLOCK`, checks the file type on the **opened descriptor** rather than on the path, and bounds the read there too. Checking the descriptor is what closes it: a `stat` followed by an `open` is a race, and these paths are re-read on every command, so an attacker gets unlimited attempts to win it. ([#457](https://github.com/yottayoshida/omamori/issues/457), [#468](https://github.com/yottayoshida/omamori/issues/468))

  Measured across seven commands × ten paths, before and after. `audit.jsonl.hwm` and `break-glass.json` hung `omamori hook-check` — **before it printed its deny verdict**, which hands the decision to the host's hook-timeout policy rather than omamori's; `config.toml` hung `doctor` and `status`. None of those three were among the five call sites originally identified, and none of them went through the shared open helper, which is the whole point: a check that lives on callers does not reach the next caller. Non-regular files are now refused by name (`… is a FIFO, not a regular file`) rather than read from, hung on, or silently treated as empty.

- **No omamori lock waits indefinitely.** Locks are taken with `LOCK_NB` and a bounded retry budget (100 × 5 ms), after which the key-store lock proceeds unlocked and the audit-log lock reports the contention. `flock` works on a read-only descriptor, so without this any local process able to open `audit.jsonl` or `audit-secret.lock` could hold a lock forever and stall `verify`, `doctor`, `report`, every guarded command, and `hook-check` — the last of these hanging *before* it prints its deny verdict, which hands the decision to the host's hook-timeout policy. Measured on all six surfaces. ([#457](https://github.com/yottayoshida/omamori/issues/457))

- **`omamori audit show | head` no longer prints `Broken pipe (os error 32)`.** Rust disables the default `SIGPIPE` handler, so a closed reader surfaced as an error and exit 1 for what is the ordinary way to read a long log. `audit show` and `audit unknown` now exit 0 when their reader goes away. ([#457](https://github.com/yottayoshida/omamori/issues/457))

- **Entries written while the key directory was unreadable are reported as unverifiable, not as tampering.** In that state omamori cannot tell which key epoch is active, so it records the entry with no HMAC. Labelling it `"default"` meant that once permissions were restored, `"default"` resolved, the recomputed hash did not match, and the entry read as tampering — permanently, since `ADR-0007` forbids rewriting it. Such entries now carry a reserved `key_id` that no keyring can hold, so they report as *cannot verify* with the actual reason. ([#457](https://github.com/yottayoshida/omamori/issues/457))
- **SECURITY.md now states what key rotation does and does not guarantee** (new "Key Rotation" section). In particular: `verify_chain` performs no monotonicity check on key epochs, so an attacker holding *any* past key — active or retired — can forge an entry that names that key and it will verify. Rotation limits which future entries a leaked key can produce; it does not invalidate a leaked key for forging entries labelled as its own epoch. Treat a leaked audit key as a permanent loss of tamper-evidence for the epochs it covers. This was already true before #457 and is unchanged by it; it had simply never been written down. ([#457](https://github.com/yottayoshida/omamori/issues/457))
- Fixing the genesis anchor (see Fixed) widens the accepted set by exactly one entry: an attacker holding only a retired key can now also forge a chain *head*, which the previous active-key-only anchor incidentally prevented. This is accepted deliberately and recorded in ADR-0008 — the protection was an implementation accident rather than a designed defense, it covered one entry out of a chain where every other position was already forgeable by the same attacker, and no version of the fix preserves it (verifying a rotation-spanning chain requires anchoring on the key the chain began with, which a forger can also name). A test pins the widened acceptance so it cannot change silently in either direction. ([#457](https://github.com/yottayoshida/omamori/issues/457))
- **New ADR-0008** (`docs/adr/0008-key-resolution-semantics-for-audit-chain-verification.md`): records the key-resolution semantics frozen for 1.0 — a value is verified with the key it names, `"default"` is an alias for epoch 1, a key we do not hold is *cannot verify* rather than *tampered*, and the verifier does not search the keyring for a key the entry did not name. ([#457](https://github.com/yottayoshida/omamori/issues/457))

- **SECURITY.md records the three residual risks 1.0 ships with, and each names its issue.** Key material is not zeroized, so key bytes survive in buffers that are dropped without being cleared — reaching them requires reading another process's memory as the same OS user, who can read `audit-secret` off the disk instead, so the boundary does not move ([#475](https://github.com/yottayoshida/omamori/issues/475)). The data directory itself gets none of the symlink care its contents do ([#486](https://github.com/yottayoshida/omamori/issues/486)). Self-protection rules match a protected verb at any argument position rather than at the verb position, which fails toward over-blocking and never toward a bypass ([#395](https://github.com/yottayoshida/omamori/issues/395)). The Dependabot audit note also gains its issue number ([#385](https://github.com/yottayoshida/omamori/issues/385)), which it had never carried. Naming each one makes the set countable from the document: for every open `security` issue, grep SECURITY.md for its number. ([#511](https://github.com/yottayoshida/omamori/pull/511))

- **Three claims in SECURITY.md were corrected rather than added to.** The parent-directory-symlink row offered `create_dir_all` being called by omamori itself as a mitigation, which rules out redirection to an attacker-named path and says nothing about a symlink already sitting at omamori's own path; `PROTECTED_FILE_PATTERNS` was offered without noting it is a hook-layer control that a direct OS operation never meets. Both claims are kept and scoped rather than dropped. Measurement also corrected the direction of the hazard: a *dangling* symlink at the data directory is not a silent redirect — `create_dir_all` returns `AlreadyExists` and creates nothing — while a symlink onto a directory that already exists passes quietly, and planting one needs write access to the parent rather than to the directory being replaced. Third, "the 7 built-in safety rules" understated the scope of Core Policy Immutability: fifteen rules carry `is_builtin` and all of them are equally immutable, the eight `omamori-*-block` rules having arrived with DI-13 after that section was written. ([#511](https://github.com/yottayoshida/omamori/pull/511))

- **A reachable over-block is now documented that was previously only a test comment.** `omamori audit show --rule key --action rotate` carries both `key` and `rotate` as flag values, which is enough for `match_all` to match the rotate guard and block a read-only command. The behaviour is pinned as accepted in the test corpus but had never appeared in SECURITY.md. ([#387](https://github.com/yottayoshida/omamori/issues/387), [#511](https://github.com/yottayoshida/omamori/pull/511))

## [0.16.0] - 2026-07-26

**Summary**: Closes out the two breaking-change items `docs/CONTRACT.md` named as planned before 1.0.0 (#175, #177). `context::normalize_path`/`evaluate_context` now require an explicit `base: &Path` (CWD-implicit resolution removed from Tier 1 path evaluation). The audit chain's `CHAIN_VERSION` flips to `2`, folding `wrapper_kind` and the four process-provenance fields (`pid`/`ppid`/`parent_process`/`cwd_hash`) into `entry_hash`'s HMAC protection on new entries, while every existing `chain_version: 1` entry remains verifiable byte-for-byte forever — no `audit.jsonl` bytes are ever rewritten. `brew`/`cargo install` binary users see one small CLI-visible change (`omamori explain` now prints the directory it evaluated against) and otherwise identical behavior; the breaking changes are Rust-library-API-only. This is a pre-1.0 verification release: the plan is to run it in production for a soak period — a real `audit.jsonl` spanning the upgrade boundary, automatic retention pruning, key rotation, and Homebrew/cargo binary coexistence, none of which CI can replicate — before cutting v1.0.0.

### Changed

- **BREAKING**: `context::normalize_path` and `context::evaluate_context` now require an explicit `base: &Path` argument instead of implicitly resolving relative paths against the process's current working directory. `context::resolve_path` is removed from the public API entirely (it had zero known callers, including on crates.io — reverse-dependency check via the crates.io API returned zero results). This closes the CWD-implicit path in Tier 1 (path-based) context evaluation and protected-file matching; Tier 2 (git-aware evaluation, `evaluate_git_context`) still spawns `git` without pinning its working directory and continues to depend on the child process inheriting the parent's CWD — out of scope for this change. A new `clippy.toml` (`disallowed-methods`) now bans `env::current_dir`/`env::set_current_dir` everywhere in the crate outside `context::process_base`, with one further named exception: `audit::provenance::ProcessProvenance::collect()` intentionally keeps its own direct `env::current_dir()` read (`#[allow(clippy::disallowed_methods)]`) because that value feeds the forensic `cwd_hash` field, which comes under the audit chain's HMAC protection as of the `CHAIN_VERSION` 2 entry below (#177 B3) and must not be rerouted through `process_base`'s different failure semantics. Every other call site was updated to pass the process's own CWD as `base`, verified byte-identical via characterization tests before and after, with one narrow behavioral exception (see Security below). The signature change itself affects only code depending on `omamori` as a library (`cargo install`/Homebrew binary users are unaffected) — see Added below for the one CLI-visible side effect. ([#175](https://github.com/yottayoshida/omamori/issues/175))
- **BREAKING**: `audit::AuditEvent` gains a new public field, `wrapper_kind: Option<String>` (present only for `BlockStructural`/materialize verdicts triggered by a transparent shell wrapper, e.g. `env`/`sudo`; `None` everywhere else, omitted from JSON when `None`), and is now marked `#[non_exhaustive]` (zero known reverse dependencies on crates.io, verified via the crates.io API). `#[non_exhaustive]` blocks cross-crate exhaustive struct-literal construction and exhaustive destructuring without `..` — it does not restrict field reads or `Deserialize`-based construction, so `audit.jsonl` remains readable by older/newer omamori binaries interchangeably regardless of this attribute. `wrapper_kind` promotes the wrapper name that was previously only embedded in `detection_layer`'s `"layer2:pipe-to-shell:{wrapper}"` suffix (which is unchanged and still present) to its own first-class field. `CHAIN_VERSION` stayed at `1` at the point this specific field landed (#177 B2) — `wrapper_kind` was not yet part of the HMAC-hashed `HashableEvent`, so a same-user attacker with direct `audit.jsonl` write access could alter it without `omamori audit verify` flagging it; the hashed `detection_layer` suffix was the only tamper-evident copy of the same fact. The `CHAIN_VERSION` 2 entry below (#177 B3) folds `wrapper_kind` into the hash on `chain_version: 2` entries — this v1-only exposure is now permanent (v1 entries never retroactively gain protection) rather than temporary. See SECURITY.md's "Channel separation" section and ADR-0006's 2026-07-25/2026-07-26 updates. ([#177](https://github.com/yottayoshida/omamori/issues/177))
- `CHAIN_VERSION` is now `2` (previously `1`) — the audit chain's HMAC-protected `entry_hash` now additionally covers `wrapper_kind` and the four process-provenance fields (`pid`/`ppid`/`parent_process`/`cwd_hash`, added unprotected in v0.13.1's #420) on any entry that declares `chain_version: 2`. **Not a breaking change under this project's [breaking-change policy](docs/CONTRACT.md#breaking-change-policy)**: every existing `chain_version: 1` entry remains verifiable byte-for-byte, forever — `omamori audit verify`/`report`/`doctor` dispatch per-entry on each entry's own declared `chain_version`, and existing `audit.jsonl` bytes are never rewritten (new: ADR-0007). A log spanning the upgrade boundary (the common case for every existing installation) is a permanent, expected mix of `chain_version: 1` and `chain_version: 2` entries, not a transitional state — `omamori audit verify`'s success message now shows a `(N v1 + M v2)` breakdown whenever a log actually contains both. The corresponding cost is permanent, not transitional either: a same-user attacker can still alter `pid`/`ppid`/`parent_process`/`cwd_hash`/`wrapper_kind` on any `chain_version: 1` entry undetected (Design A, ADR-0006) — this residual risk does not shrink over time, since v1 entries already on disk never become v2. See SECURITY.md's "Process Provenance" and "Channel separation" sections (updated) for the full v1/v2 split, and ADR-0006/ADR-0007 for the design rationale. ([#177](https://github.com/yottayoshida/omamori/issues/177))
- **BREAKING**: `audit::VerifyResult` gains two new public fields, `v1_entries: u64` and `v2_entries: u64` (per-version entry tallies backing the mixed-counter CLI message below), and is now marked `#[non_exhaustive]` (zero known reverse dependencies on crates.io, verified via the crates.io API — same precedent #177 B2 set for `AuditEvent`). `#[non_exhaustive]` blocks cross-crate exhaustive struct-literal construction and exhaustive destructuring without `..`; `VerifyResult` also gains `#[derive(Default)]` as the supported construction path. (QA Phase 8, [#177](https://github.com/yottayoshida/omamori/issues/177) B3)
...
- **`omamori audit verify` gains exit code 4** (in addition to the existing 0/1/2/3) for an audit-chain entry declaring a `chain_version` this binary doesn't recognize (a future format, or a downgrade after a newer entry was written). This is reported distinctly from tampering (exit 1) across `verify`/`report`/`doctor` — an unrecognized version is not, by itself, evidence of tampering. `[audit] strict = true` now blocks the triggering command when the audit-chain tail can't be recorded for this reason, the same as it already does for any other unrecordable event (secret unavailable, disk full, permissions). `CHAIN_VERSION` was unchanged (still `1`) at the point this exit-code-4 handling landed (#177 B1) — it only added handling for a version *other* binaries or future releases might write; every existing v1 chain verified byte-identically to before. (`CHAIN_VERSION` is `2` as of the entry below, #177 B3 — the same exit-4 handling now also covers any version this binary doesn't recognize, `3` and up.) See SECURITY.md's new "Forward-Unknown Chain Versions" section for the accepted residual risk: a same-user attacker who can already write `audit.jsonl` can plant one such entry ahead of a real deletion/reorder/truncation to soften that report from exit 1 to exit 4, but cannot destroy the evidence (the high-water-mark is preserved untouched) or hide anything earlier in the file, and detection resumes in full once the planted entry is removed. ([#177](https://github.com/yottayoshida/omamori/issues/177))
- **`omamori audit verify`'s success message now shows a version breakdown** — `N entries verified, chain intact. (X v1 + Y v2)` — whenever a log actually contains both `chain_version: 1` and `chain_version: 2` entries; a single-version log (before upgrading, or long after) gets no added parenthetical. ([#177](https://github.com/yottayoshida/omamori/issues/177))
- **New `invariants-check` CI invariant `chain-version-doc-sync`**: verifies SECURITY.md's `chain_version` schema-table row states the actual current `CHAIN_VERSION` constant from `src/audit/chain.rs`, so a future version bump that updates the constant but forgets the doc line is caught in CI instead of shipping silently stale (mirrors the existing `faq-doc-sync`/`contract-doc-sync` pattern). ([#177](https://github.com/yottayoshida/omamori/issues/177))
- **New ADR-0007** (`docs/adr/0007-no-in-place-rewrite-of-existing-audit-entries.md`): records, as an explicit decision rather than an implicit assumption, that existing `audit.jsonl` entries are never rewritten in place — including as part of a future `CHAIN_VERSION` migration. ([#177](https://github.com/yottayoshida/omamori/issues/177))
...
- **Hardened**: a legacy-shaped audit entry (no `chain_version` at all) appearing *after* real chain entries have started now fails closed (`omamori audit verify` exit 1) instead of being silently counted as benign pre-#164 history. Legacy entries never participate in `prev_hash`/`seq` continuity tracking, so without this an attacker could splice unaudited content into the middle of an otherwise fully-verified chain by making it look legacy. Legacy entries at the head of the file — genuine history predating chain tracking — are unaffected; this only changes behavior for the shape no legitimate omamori installation produces (chain, then legacy, in that order). ([#177](https://github.com/yottayoshida/omamori/issues/177))
- **Hardened**: on `chain_version: 2` entries, a same-user attacker with direct `audit.jsonl` write access but without the HMAC secret can no longer alter `pid`/`ppid`/`parent_process`/`cwd_hash`/`wrapper_kind` without `omamori audit verify` detecting it (`broken_at`) — these 5 fields are now part of the HMAC-hashed `HashableEventV2`. This protection is **not retroactive**: `chain_version: 1` entries (every entry written before upgrading, permanently) remain exactly as unprotected for these fields as before — existing `audit.jsonl` bytes are never rewritten (ADR-0007). An attacker who has also read the secret remains able to forge a self-consistent chain regardless of `chain_version`, same as documented elsewhere in SECURITY.md's Defense Boundary. See `docs/adr/0006-process-provenance-excluded-from-entry-hash.md`'s 2026-07-26 update for the full before/after. ([#177](https://github.com/yottayoshida/omamori/issues/177))
...
- **Internal**: extracted two shared CLI-flag-value-parsing helpers (`flag_value`/`flag_value_str` in `src/util.rs`) and migrated 13 hand-rolled "get the next arg or error" call sites across `install`/`setup`/`status`/`doctor`/`audit`/`report` to use them, removing 13 places where a copy-pasted `index += 2` could silently drift into an off-by-one. No observable behavior change — every error message, non-UTF8 value/flag-name handling, and adjacent-flag greedy value consumption is preserved and pinned by new characterization tests. `config_cmd.rs`'s 6 already-locally-deduplicated call sites were intentionally left for a follow-up issue, closed by #451 below. ([#392](https://github.com/yottayoshida/omamori/issues/392), [#377](https://github.com/yottayoshida/omamori/issues/377))
- **Internal**: `config_cmd.rs`'s `config add` parser now uses the same shared `flag_value_str` helper for its 6 flag-value call sites (`--command`/`--action`/`--match-any`/`--match-all`/`--destination`/`--message`), replacing a locally-duplicated `take_value` closure. No observable behavior change — every error message, non-UTF8 value folding, and adjacent-flag greedy value consumption is preserved and pinned by new characterization tests. The issue's accompanying proposal to relax the helpers' `FnOnce` bound to `Fn` was investigated and rejected: `Fn` is a *stricter* trait bound than `FnOnce` in Rust's trait hierarchy, so the change would have been a breaking tightening across all 19 call sites, not a relaxation, with no functional benefit (`err` is called at most once per call site). ([#451](https://github.com/yottayoshida/omamori/issues/451))
...
- **`omamori --version` now appends the product-contract version**: e.g. `omamori 0.15.1 (Contract v1)`, sourced from a single `CONTRACT_VERSION` constant. The `contract-doc-sync` invariant (`scripts/check-invariants.sh`) now pins this constant against `docs/CONTRACT.md`'s header table (`| Contract version | vN |`) — a mismatch fails CI, so the tool's observable value cannot silently drift from the document that defines it. ([#422](https://github.com/yottayoshida/omamori/issues/422))
- **`omamori doctor` now checks `codex-pretooluse.sh` for hash and version drift** (#327 extended to Codex CLI): a new `codex-pretooluse.sh` `CheckItem` mirrors the existing `claude-pretooluse.sh` check exactly, including the `[version drift: ...]` advisory suffix when the installed wrapper's embedded `# omamori hook v{X}` comment disagrees with the running binary. This CheckItem WARNs whenever the wrapper is absent from omamori's own hooks directory, independent of whether Codex CLI (or `~/.codex`) is detected on the machine — `install --hooks` itself still only writes the wrapper when `~/.codex` exists, so a fresh install without it will now show a permanent Warn here rather than silently omitting the check. ([#381](https://github.com/yottayoshida/omamori/issues/381))
- **`omamori doctor`'s Cursor snippet check now detects version drift and exe-resolution failures the same way the Claude/Codex checks do** (#327 extended to Cursor): previously, when the currently-running omamori exe couldn't be resolved, the Cursor check silently reported `"(present, hash check skipped)"` with no version information — the exact masking scenario (e.g. a broken Homebrew Cellar symlink) #327 closed for Claude/Codex was still open for Cursor. A new `parse_cursor_snippet_version` extracts the version embedded in the snippet's JSON `_comment` field (`"Generated by omamori v{X}. Merge into .cursor/hooks.json"`), and the exe-resolution seam is now injectable (`ExeResolver`, matching the Claude/Codex pattern) so both the resolution-failure and hash-mismatch branches surface a `[version drift: ...]` suffix. The two dangling-exe-path branches deliberately keep their existing dangling-only message — a broken exe path is already the more actionable diagnosis there. ([#382](https://github.com/yottayoshida/omamori/issues/382))

### Changed

- **Version-drift advisory wording is now format-neutral**: `hook_version_drift_suffix`'s "no version comment"/"hook script's version comment is unparseable" messages (previously assuming a shell-comment source) now read "no version metadata found"/"version metadata is unparseable", since the same detection logic is now shared by Claude/Codex's shell-comment hooks and Cursor's JSON `_comment` field (#381/#382). No change to when the advisory appears, only its wording.

### Fixed

- **`AuditConfig::validate()` now neutralizes a relative `audit.path` override**: a relative `[audit] path = "audit.jsonl"` in a user- or AI-edited `config.toml` previously made every audit operation (`rotate_key`, `verify_chain`, `show_entries`, `AuditLogger::from_config`) resolve against the process CWD, reintroducing the exact #210-class CWD-scatter hazard (the HMAC secret and audit log written into whatever directory the command happened to run from — e.g. an AI agent's working git repo) that #371 closed for the `path = None` case, this time through the explicit-override path `resolved_audit_path()` never validated. `validate()` now drops a non-absolute `path` to `None` with a loud, actionable warning surfaced on every command via `emit_config_warnings` (now wired into all 5 `omamori audit *` subcommand entry points — `verify`/`show`/`unknown`/`key rotate`/`hash-cwd` — which had never called it, silently defeating this design's own visibility rationale for the exact commands issue #439 names), so `resolved_audit_path()` falls through to the HOME-derived default or fails closed when HOME is unusable — the same fail-close-with-recovery-hint pattern as #371/#373 and the `is_absolute()` filter `context::home_dir()` already applies to HOME and `config::validate_destination` applies to a rule's relative `destination`. `resolved_audit_path()` also gained a second, silent `is_absolute()` filter as defense-in-depth for any future caller constructing an `AuditConfig` without going through `validate()`. `omamori init`/`setup` never wrote a relative `audit.path`, so this was not exploitable through the shipped tooling — it hardens against hand- or AI-authored configs. If you were (accidentally) relying on a relative `audit.path` resolving against a particular CWD, that override is now ignored: audit logging falls back to the HOME-derived default (`$HOME/.local/share/omamori/audit.jsonl`), or is disabled if `HOME` is also unusable — set an absolute `audit.path` in config.toml to keep a custom location. `validate()` is the single choke point every audit entry point already routes through, so no call-site changes were needed beyond the warning wiring above. Absolute `audit.path` values, and `HOME` itself, remain trusted verbatim — this closes only the relative/empty-path case, not a deliberate absolute-path redirection or `HOME` manipulation (see SECURITY.md's Defense Boundary section). Discovered during PR #438 (#371) Phase 6-B adversarial review. ([#439](https://github.com/yottayoshida/omamori/issues/439))
- **`docs/CONTRACT.md` and `docs/FAQ.md` are now included in the published crate tarball**: both are linked from README, but neither was in `Cargo.toml`'s `include` list or `scripts/check-crate-contents.sh`'s allowlist, so a `cargo install omamori` user got a README pointing at files the package didn't contain. `docs/FAQ.md` had been missing since PR #399 (2026-07-13); `docs/CONTRACT.md` since its introduction in #401. Both are now added as exact allowlist entries (not a `docs/*.md` glob, to keep `docs/`'s non-shippable files — e.g. `.DS_Store`, `defense-boundary.json` — out of the crate). ([#424](https://github.com/yottayoshida/omamori/issues/424))
- **README's `omamori doctor` sample output was stale**: the `[Layer 2] Hook defense` count still read `4/4` after the `codex-pretooluse.sh` CheckItem was added, undercounting a healthy install's actual check total. Corrected to `5/5`, confirmed against a real `omamori doctor` run in an isolated sandbox.
...
- **`installer.rs`/`engine/shim.rs`**: unified 2 of 3 divergent exe-injection DI-seam shapes onto `Option<&Path>`. `regenerate_hooks_for_exe` previously took a mandatory `stable_exe: &Path`; it now takes `exe_override: Option<&Path>` matching `ensure_hooks_current_at_with_verifier_and_exe`'s existing shape, with the `None`-branch resolution logic (`resolved_current_omamori_exe()`, fail-closed on error) folded in and `regenerate_hooks_with_verifier` reduced to a one-line wrapper. A new regression test (`hooks_current_rejects_dev_build_path_injected_via_exe_override`) closes a negative-control gap flagged during review: the shim-side seam previously had no direct test proving a dev-build path injected via `exe_override` gets rejected rather than silently bypassing the #354 guard. `docs/adr/0004-hook-exec-path-provenance-policy.md` promoted to Accepted and updated to document the third seam (`auto_setup_codex_if_needed_with_exe`, already `Option<&Path>`-shaped) and its intentionally different resolution behavior. ([#376](https://github.com/yottayoshida/omamori/issues/376))
- **`omamori setup --dry-run`**: now previews the #354 implicit-dev-build-path rejection instead of silently reporting "would install hooks" for a run that would actually be rejected. A dry run from a dev checkout with no `--source` shows a `WARNING` on the `[1/3]` Hooks line, an advisory that the remaining steps won't be reached, and `(not reached)` labels on the `[2/3]`/`[3/3]` section headings — added during `/simplify` review after an earlier draft's advisory-line-only approach left the preview self-contradictory (announcing steps wouldn't be reached, then detailing them anyway). `--source` still bypasses the gate with no warning, matching `install()`'s own bypass; `current_exe()` resolution failure falls back to the unwarned preview rather than changing dry-run's exit code. The preview and `install()`'s actual enforcement now share one predicate (`would_reject_implicit_dev_build`) and one path-resolution step (`resolve_source_target`), closing a known, previously-accepted gap documented in ADR-0004. ([#380](https://github.com/yottayoshida/omamori/issues/380))
- **BREAKING: `installer::default_base_dir()` now returns `Option<PathBuf>`** (was `PathBuf`, fail-open to `.` when `HOME` was unset/empty/relative). This closes the last of three independently-diverging HOME resolvers flagged in #373: `context::home_dir()` (strictest — `None` on unset/empty/relative, unchanged), the now-deleted `installer::home_dir()`, and `default_base_dir()` itself all now route through the single fail-close `context::home_dir()`. Call sites split into three classes: `install`/`uninstall`/`status`/`doctor`/`setup` (Class A) now return a loud, actionable `AppError::Config` — `cannot resolve a base directory — HOME is unset, empty, or relative (must be absolute)` plus a `--base-dir`/`HOME` recovery hint — instead of silently writing `./.omamori` into whatever directory the command happened to run from (the #210 CWD-pollution failure mode this PR closes); `config init`/`config add`/`config disable`/`config enable` (Class B) already succeed on `XDG_CONFIG_HOME` alone and now just silently skip their best-effort integrity-baseline refresh when `HOME` is unusable, rather than erroring a config write that already landed; and the PATH-shim hot path (`run_shim`, Class C) skips only self-repair housekeeping (hook/settings regen, integrity canary, the audit-throttle sentinel) — the actual protection decision (`run_command`) runs unconditionally regardless of `HOME`, pinned by new regression tests (`run_shim_still_blocks_dangerous_command_when_home_unset`, `run_shim_skips_housekeeping_without_cwd_writes_when_home_unset`). A related, independently-discovered gap in `context.rs`'s `~/`-tilde expansion (it read `HOME` directly rather than through the fail-close resolver, so `HOME=""` silently reinterpreted `~/foo` as the relative path `foo`) is fixed in the same PR — an unusable `HOME` now leaves the literal `~/foo` string untouched instead of guessing. `audit_warn_sentinel_path` (#372) is folded into this unification — it now delegates to `default_base_dir()` instead of duplicating its own path logic, closing #372 as a side effect. 0.x version, so this ships as a minor bump per existing policy. ([#373](https://github.com/yottayoshida/omamori/issues/373), [#372](https://github.com/yottayoshida/omamori/issues/372))
- **BREAKING: `audit::rotate_key()` now takes `path: &Path` instead of `config: &AuditConfig`** — the last audit-subsystem function still resolving its own path internally, and the only one still fail-open (its removed `secret::default_audit_path()` helper fell back to a CWD-relative path when `HOME` was unset/empty/relative — the same #210-class bug #373 closed elsewhere). Callers now resolve through `resolved_audit_path()` (the fail-close resolver `verify.rs`/`report.rs`/`provenance.rs`/`doctor.rs` already use) before calling `rotate_key`, exactly as issue #371 specified. This also removes `audit_cmd.rs`'s `rotate_key_would_use_cwd_fallback` — a caller-side predicate that re-implemented `rotate_key`'s internal HOME-resolution logic from outside just to guess whether it would hit the CWD fallback, which could silently drift from the logic it was mirroring. `rotate_key`'s own fail-close boundary is now enforced by the type system instead of a duplicated runtime guess. User-visible behavior (when `omamori audit key rotate` errors, and its exact message) is unchanged — verified to be byte-identical across the HOME unset/empty/relative/absolute boundary. A new integration test (`tests/cli.rs`) proves the wiring end-to-end: with `HOME` unset, `rotate` exits non-zero and creates no `audit-secret`/`audit-secret.*.retired` file in the CWD. ([#371](https://github.com/yottayoshida/omamori/issues/371))
- **docs**: reconciled SECURITY.md's Defense Boundary Matrix status legend, RELEASE.md's v1.0 release gate, `docs/defense-boundary.json`'s `status_vocabulary`/`layer2_note`, and README's Tool Compatibility table with `docs/CONTRACT.md`'s Tier 1/Tier 2 model (#401) — all four previously described Codex CLI (and, in README's table, Cursor) with more verification confidence than Tier 2 ("expected to work, not contractually guaranteed, no continuous acceptance verification") actually grants: SECURITY.md's legend and README's table used the same "tested"/"supported" wording as for Claude Code with no tier qualifier, and RELEASE.md gated Codex CLI on the identical live-path acceptance criterion as Claude Code. Codex CLI stays Tier 2 going forward — this is a documentation-only wording fix, not a scope or CI change. Closes CONTRACT.md's previously-disclosed "Known inconsistency" note. ([#423](https://github.com/yottayoshida/omamori/issues/423))

## [0.14.1] - 2026-07-21

**Summary**: Publishes a machine-readable, CI-enforced projection of SECURITY.md's Defense Boundary Matrix for external tooling and dashboards (#405).
...
- **SECURITY.md Disclosure timeline and Redaction policy**: a 90-day embargo upper bound for confirmed in-scope bypasses, framed as the reporter's right to disclose (not a maintainer fix commitment) and immune to being reset by an interim maintainer acknowledgment. Documenting a bypass as a structural limit is a valid resolution in its own right, not a stall. Redaction policy: no discretionary tier — once resolved, a bypass follows the existing Known-bypass-becomes-row rule in full — with one narrow carve-out for bypasses chaining through an unfixed third-party vulnerability (the third-party detail is held until fixed or the embargo lapses; omamori's own mitigation and the existence of the gap are still disclosed on the normal timeline, and using the carve-out requires a known-limit entry recording it). A new Corpus PR review criterion gives reviewers a citable basis to reject exploit walkthroughs in public PR descriptions or commit messages. ([#404](https://github.com/yottayoshida/omamori/issues/404))
- **`docs/reference-architecture.md`**: a responsibility-boundary table between omamori's semantic layer and provider sandboxes' OS boundary, with concrete examples of what each layer catches that the other structurally cannot; a bidirectional anti-claim (neither layer substitutes for the other); and per-tool setup notes for Claude Code, Codex CLI, and Cursor, cross-referencing each tool's CONTRACT.md Tier 1/2 status so the notes don't imply a contractual commitment the tier doesn't back. Linked from README's Sandbox complementarity section and CONTRACT.md's Authority Map. ([#407](https://github.com/yottayoshida/omamori/issues/407))
- **`docs/evaluation-kit.md`** and a GitHub issue form (`.github/ISSUE_TEMPLATE/evaluation-feedback.yml`): a self-contained 30-day checklist for external evaluators, reporting only aggregate `omamori report`/`doctor` output — no raw command history. The form has no public field for reporting a missed accident; that routes to SECURITY.md's private vulnerability reporting instead, to avoid disclosing an unfixed gap in a public issue. An early-stop report (disable/uninstall + why) is treated as a complete, valid submission on its own — arguably the most useful signal the kit can capture. ([#408](https://github.com/yottayoshida/omamori/issues/408))
- **`extdocs-doc-sync` invariant** in `scripts/check-invariants.sh` (mirrored from `contract-doc-sync`, scoped to the two new docs above): resolves SECURITY.md/CONTRACT.md/README.md anchor references, each doc's own same-document anchors, `omamori` CLI subcommand references (from both fenced ```bash blocks and backtick prose), README backlink existence, and forbidden over-claim language. Each of the two target docs is checked independently — a missing file does not suppress checks on the other.
- **`scripts/verify-claims.sh`** (new `claims-check` CI job): README's Verifiable Claims table maps 5 rows to CI job ids, CONTRACT.md guarantee IDs, and a "Verify yourself" command per row — this script keeps that mapping from silently rotting rather than re-proving each claim's behavior (that's already covered by named cargo tests). Five checks: the table↔CI-job-id↔G-ID↔"Verify yourself"-text mapping stays in sync, with no `continue-on-error: true` escape hatch on a mapped job (M1); a Cargo.lock dependency allowlist and a hook-decision-path source-token scan close claim 4's one true gap — "no network dependency, no model call" had zero existing machine enforcement (M2/M3); the honest-limitation prose for claims 2/3/4 hasn't been silently deleted (M4); and the 13 named cargo tests backing claims 1/2/3/5 still exist and aren't `#[ignore]`d (M5). `--self-test` mode injects one violation per check into a temp copy (never the product's own files) and asserts the exact resulting FAIL — 7 passes, run every CI invocation, mirroring `test-isolation-canary-self-test`'s inverted-control pattern. Claim 5's "Verify yourself" cell now runs `CLAUDECODE=1 omamori config disable rm-recursive-to-trash`, which actually exercises the AI-environment-gated block the claim describes (the previous `omamori --help` proved nothing); `docs/CONTRACT.md`'s G-5 Verify block and G-3 Verify block (which was missing `omamori status`) were brought into sync with README as part of this work. New test `hook_deny_blockstructural_creates_audit_entry` (`tests/hook_integration.rs`) closes claim 2's one gap with no existing direct assertion — the `BlockStructural` `HookCheckResult` variant's audit-chain write was implemented but untested until now. ([#403](https://github.com/yottayoshida/omamori/issues/403))
...
- **Layer 1 (PATH shim) audit events carry process provenance** — `pid`, `ppid`, `parent_process` (the launching process's resolved exec path via `proc_pidpath`, never `argv[0]`), and `cwd_hash` (a domain-separated HMAC of the working directory at collection time, distinct from `target_hash`). Collection is fail-open: any failure degrades the affected field to `None` and never blocks the guarded command or the audit append. This was added after a real incident where a repeating destructive command pair fired 16 times in one day and could not be traced back to its launching process using `target_hash` alone. Provenance is forensic best-effort, not an authenticated identity — a shell hop, deliberate orphaning, a double-fork, or a crafted binary name can each launder or defeat it (documented as a limitation, not a guarantee). Layer 2 (hooks) is deliberately out of scope — the motivating incident had zero Layer 2 events. ([#420](https://github.com/yottayoshida/omamori/issues/420))
- **`omamori audit hash-cwd <path>`**: hashes a candidate directory against every HMAC key in the keyring (active plus any retired from a prior rotation) and both the raw and symlink-resolved forms of the path, so an investigator can grep the audit log for a match without knowing which key was active or whether the logged value used the resolved path.
- **`docs/adr/0006-process-provenance-excluded-from-entry-hash.md`**: records the decision to keep the four new provenance fields outside `HashableEvent` (`CHAIN_VERSION` stays at `1`) — they are advisory, not tamper-evident, with the tamper-evidence upgrade tracked under issue #177's future `CHAIN_VERSION` 1→2 migration. Existing audit chains remain verifiable unchanged; `GOLDEN_ENTRY_HASHES` and the `HashableEvent` field-order golden test (GR-002) are unaffected.
- **`config disable`/`config enable`/`config add` now append an audit event on success** — `action` gets new string values (`config-disable`/`config-enable`/`config-add`), `detection_layer` is `config-mutation`, `provider` is `cli`. Only the rule name and command verb are recorded, never a rule's content (destination, match patterns, message), consistent with the existing `target_hash` HMAC-hashing-of-paths privacy design. `override disable`/`override enable` are not yet covered — disclosed as a follow-up in SECURITY.md. ([#394](https://github.com/yottayoshida/omamori/issues/394))
- **GitHub Private Vulnerability Reporting is now enabled**, with the intake path documented in SECURITY.md (channel, response-time expectation, in-scope/out-of-scope criteria) and cross-referenced from README. ([#402](https://github.com/yottayoshida/omamori/issues/402))

### Fixed

- **`omamori audit key rotate` is now covered by a Phase 2 backstop rule** (`omamori-audit-key-rotate-block`), closing a gap where `env -i` could strip detector env vars and bypass the runtime `guard_ai_config_modification` check that was previously the only protection. Mirrors the 7 existing `omamori-*-block` self-protection rules and is registered as non-bypassable via break-glass. Core rule count is now 15 (was 14). ([#387](https://github.com/yottayoshida/omamori/issues/387))
- **`move_to_dir`'s timestamped staging subdirectory is hardened against symlink planting** — it previously used non-exclusive `create_dir_all` with no symlink check, so a symlink pre-planted at the predictable `<destination>/<unix-secs>` path could redirect the quarantine `rename` to an attacker-controlled location. Now uses exclusive `create_dir` with a high-entropy random suffix (`{timestamp}-{16-hex}`), retrying with a fresh random suffix (never a predictable sequential one) up to 8 times before failing closed with zero files moved. A same-user active-watcher race between the exclusive create and the subsequent renames remains a documented residual risk (SECURITY.md), deferred to future fd-relative hardening. ([#410](https://github.com/yottayoshida/omamori/issues/410))
...
- **`validate_destination`'s blocked-prefix check no longer silently passes a nonexistent path under a blocked system prefix** (e.g. `/etc/made-up-name`) — it previously required `Path::canonicalize()` to succeed on the whole path, which requires the whole path to already exist. It now walks up to the nearest existing ancestor, canonicalizes that (resolving any symlink in the ancestor chain), rejoins the missing tail, and lexically folds the result before a segment-aware `Path::starts_with` comparison — which also fixes 7 false positives the previous `str::starts_with` comparison produced (`/usr-local`, `/etcetera`, `/vartmp`, `/binary`, `/private-data`, `/Systemx`, `/sbinx`). Runtime `move_to_dir` is unaffected — it remains the fail-close authority and shares only the resolved-path/blocked-prefix comparison, not the validate-time leniency. ([#391](https://github.com/yottayoshida/omamori/issues/391))
- **`config disable`/`config add` no longer diverge on config.toml shapes other than `[[rules]]` array-of-tables** (a bare `[rules]` table, a scalar `rules` value, or an inline array `rules = [{...}]`) — `disable`'s previous fallback silently overwrote whatever was there via `doc.insert`, while `add` correctly refused. Both (and `config enable`) now share a single `get_or_create_rules_array` helper that always refuses non-destructively on these shapes, leaving config.toml byte-for-byte unchanged. ([#389](https://github.com/yottayoshida/omamori/issues/389))
- **`config disable`/`config enable` now accept custom (non-built-in) rule names**, not just the 14 built-in rule ids — a rule created via `config add` can finally be toggled without hand-editing `config.toml`. `override disable`/`override enable` are unaffected and remain core-only. A custom rule's disable is an in-place `enabled = false` rewrite of its existing entry (never a stub push, which would risk a first-name-wins data-loss trap against a duplicate raw entry). ([#388](https://github.com/yottayoshida/omamori/issues/388))
- **`config disable`/`config enable`'s idempotence check now reads the raw `enabled` toggle, not the merged/validated effective state** — previously, a rule effectively disabled only by a validation issue (e.g. a `move-to` destination resolving under a blocked prefix) with no explicit raw `enabled = false` was misreported as "already disabled," skipping the write; fixing the validation issue later would then silently reactivate the rule despite the earlier `disable` having reported success.
- **`config enable` no longer falsely reports "already enabled" for a core rule disabled via `omamori override disable`** — that command writes to `[overrides]`, which `config enable` never touched, so it previously no-op'd while claiming success. It now redirects to `override enable`.
- **The README's recommended `move-to` quarantine destination is no longer `/tmp/omamori-quarantine/`** — on macOS, `/tmp` resolves to `/private/tmp`, which the #391 fix (correctly) rejects as a blocked system path; the destination was also a poor fit for a "moved here in case you need it back" quarantine location, since `/tmp` is conventionally wiped on reboot. Replaced with a home-directory-based example (`/Users/you/.omamori-quarantine/`) across README.md, `config.default.toml`, and the `omamori init`-generated config template. See [ADR-0005](docs/adr/0005-no-tmp-exception-for-blocked-destination-prefixes.md) for why `/tmp` is not allowlisted instead.
- **`config list`'s Action column is wide enough to keep the Status/Source columns aligned for a realistic `move-to` destination** — the previous fixed 16-character width broke alignment for any destination longer than that, including the README's own (now longer) recommended example.
...
- **Atomic-write pre-creation race closed across all sites, symlink rejection unified** — new `atomic_file` module provides two entry points (`atomic_write_with_mode` for replace-if-exists, `atomic_create_new` for no-clobber contracts), both unconditionally enforcing `create_new` (O_EXCL) + `O_NOFOLLOW` + mode set at creation time + CSPRNG-random temp names + `fsync` (file and parent directory) + cleanup-on-error, with no caller-facing knob able to weaken any of it. Migrated: heartbeat and audit-warn sentinel writers, `break_glass::write_state` (previously the weakest of the eight — no `O_NOFOLLOW`, permissions set via `chmod` after opening rather than at creation), `integrity::write_new_file`/`write_baseline`, `config_cmd::mutate_config`, `config::write_default_config`, and all 18 atomic-write call sites in `installer.rs` (previously the only implementation with no `fsync` at all, despite issue #307 assuming it was the strongest of the eight). `config::reject_symlink` is now a thin adapter over the shared `atomic_file::is_symlink` primitive (#311); its public `reject_symlink_public` signature is unchanged. `installer.rs`'s 4 script-writing sites (Claude/Codex hook wrappers) now set `0o755` at file creation instead of the old rename-then-`chmod` dance, closing the window where a freshly regenerated hook script briefly existed non-executable. `installer.rs`'s other 10 data-writing sites move from umask-derived permissions to a fixed `0o600` — a user-visible permissions tightening. Landed across two PRs: [#343](https://github.com/yottayoshida/omamori/pull/343) (heartbeat/audit-warn/break-glass/integrity/config_cmd) and [#345](https://github.com/yottayoshida/omamori/pull/345) (config/installer + symlink-rejection unification). ([#307](https://github.com/yottayoshida/omamori/issues/307), [#311](https://github.com/yottayoshida/omamori/issues/311), [#322](https://github.com/yottayoshida/omamori/issues/322))
...
- **break-glass activation now requires an interactive terminal** — `omamori break-glass --rule <id>` previously read its `[y/N]` confirmation from stdin with no TTY check, so `printf 'y\n' | omamori break-glass --rule <id>` satisfied it non-interactively without a human ever seeing the prompt. The confirmation now checks `stdin.is_terminal()` before any file I/O, audit logging, or state change (mirrors `setup`'s TTY-check discipline); non-interactive attempts are refused and logged to the audit chain as a denied attempt (`provider: "non-interactive"`, distinct from the `provider: "human"` used for genuine activations). No `--yes`/`--force` escape was added — this restores the design intent already stated in [0.11.1] below ("no `--yes` flag to prevent AI activation"), which the implementation had drifted from. `--status` and `--clear` are unaffected (read-only / protection-restoring). The interactive confirmation was never one of the "two independent gates" against AI activation — those are `guard_ai_config_modification()` and the DI-13 builtin rule; the TTY check is a third, weaker defense-in-depth layer, and this fix does not change that. ([#319](https://github.com/yottayoshida/omamori/issues/319))
...
- **`RedirectToken` enum** in `src/unwrap.rs`. Three variants (`PureWithOperand`, `Concatenated`, `NotRedirect`) with `token_span()` arity (`PureWithOperand` → 2, `Concatenated` → 1, `NotRedirect` → 0) and `classify(token)` constructor with longest-prefix-first matching. Single source of truth for redirect classification; designed to migrate to `src/parser/redirect.rs` in v0.10.0 (single-SoT extraction tracked separately).
- **`strip_single_fd_digit` helper** for single-digit fd prefix recognition (`n<`, `n>`, `n>>`, `n<>`, `n>|`, `n<<-`, `n<<`, `n<&`, `n>&` for n=0-9). Multi-digit fd (`10<file`) explicitly out of scope, deferred to v0.10.0 with `src/parser/` extraction.
- **22 acceptance test rows** (`R-1`..`R-22`) in `ACCEPTANCE_TEST.md` covering 9 sentinel triggers (#212 reproducer × 9 wrappers), 7 FN-regression boundaries (Codex Round 1+2 counterexamples: `&>>`, `<>`, `<<-`, `3<`, `2<>`, `&>>file`, `2>&1`), and 6 representative V-027 proc-sub + wrapper rows. Full-coverage stochastic via existing proptest generator.
- **20 new `HOOK_DECISION_CASES`** in `tests/hook_integration.rs`: 11 redirect-axis closure rows (`redirect-axis-*-block`, including 6 wrapper-variant rows added per Codex R1 P1 coverage-gap fix) + 9 V-027 proc-sub + wrapper regression rows (`v027-proc-sub-*-block`).
- **16 new unit tests** in `src/unwrap.rs::tests`: `redirect_token_classify_table` (64-entry exhaustive classification table, including fd-dup separated-operand exact rows added per Codex R1 P0 fix), `redirect_token_token_span_arity`, `redirect_token_is_redirect`, 8 FN-regression boundary tests pinning the Codex Round 1+2 counterexamples as Block (including `pipe_to_env_bash_fd_dup_separated_dash_s_blocks` for the `<&` / `>&` exact form added per R1 P0), 5 FP-pin tests (3 standard + 2 Phase-2-architect-discovered: quoted-literal `'2>&1'` block-still-fires + malformed-redirect `2>&` fail-close, minus the deliberately-omitted `>|` FP-pin — see Removed below).
- **`SECURITY.md ### Forensic semantics`** subsection under Layer 2 Deny Coverage. Documents the contract: an audit row exists for every Claude Code / Codex `hook-check` Layer 2 deny verdict (`BlockMeta` / `BlockRule` / `BlockStructural`); the absence of a row in those provider paths implies `Allow` (or, in the limit, a missed-detection bypass). Provider scope: Cursor hooks emit stderr-only deny messages without an audit-log append, so absence in the audit log does not imply Cursor deny did not happen. HMAC chain integrity is not the same as forensic completeness — the chain protects against tampering with recorded events, not against under-recording.
- **`SECURITY.md A. Closed in v0.9.x series` v0.9.8 row** documenting the redirect-axis closure with the same precedent format used for v0.9.5 wrapper closure and v0.9.6 env-S closure (no GHSA filing per maintained operating-model consistency).
- **Performance benchmarks** in `benches/` using `criterion = "0.8.2"` ([#124](https://github.com/yottayoshida/omamori/issues/124)). Three suites (`hook_check`, `unwrap_parse`, `audit_append`) cover the JSON-parse → command-check pipeline, command tokenization, and audit ledger append (flock + tail-seek chain read + HMAC entry hash + JSON serialize + fs append). Excluded from the crates.io tarball via the `Cargo.toml [package] include` allowlist (Invariant #4); the `crate-contents-guard` CI job is the defense-in-depth layer. Regression budget for v0.9.8 is observe-only — bench numbers surface in the PR description but no CI gate fires on regression. Relative-comparison gating shifts in v0.9.9+ once 3-4 release baselines exist. CI runs `cargo bench --no-run` as a new `bench-compile` job (compile-check only; CI runner variance makes wall-clock measurements unreliable, so measurement stays local).
- **README "Performance" claim**: hook check completes in under 0.1 ms (block ~1 µs, allow ~57 µs measured via the new bench harness; both are dwarfed by per-command subprocess startup cost). Replaces the implicit "no published numbers" stance that prompted [#124](https://github.com/yottayoshida/omamori/issues/124).

### Changed

- **`is_pure_redirect_op` / `is_concatenated_redirect` deleted** in favor of `RedirectToken::classify`. Caller sites (`unwrap_transparent` redirect-skip block, `strip_leading_noise` body, new arity-aware skip in `classify_shell_args`) now route through the enum; idx advance is `idx.saturating_add(kind.token_span()).min(len)` uniformly.
- **`classify_shell_args` arity-aware skip inserted** at the loop head (right after `let t = args[idx]`), before option parsing. Bash treats redirects as shell metadata, not positional args or flags — they don't satisfy the "first script-path positional" branch and don't toggle the stdin signal. Closes Round 1 Axis 5 P0 (`bash 2>&1 -s` previously consumed `-s` as script path) and Round 2 Axis 1/Axis 3 P0 (`bash &>> log -s` previously consumed `log` as script path because the bool-pair misclassified `&>>` as Concatenated).
- **`segment_has_stdin_redirect` (`src/unwrap.rs`)** keeps its own literal redirect set with a new `// TODO(v0.10.0)` cross-reference. The function answers a different question (does the segment have an explicit stdin-redirect that exempts it from pipe-to-shell) and the conservative subset reduces FP risk for the C-2/P0-2 test rows; unification with `RedirectToken` deferred to v0.10.0 with `src/parser/` extraction.

### Removed

- **`>|` FP-pin test deliberately omitted**. `normalize_compound_operators` (byte-stream pre-tokenization at `src/unwrap.rs:218`) space-wraps any `|`, splitting `>|` into a synthetic pipe segment boundary before `RedirectToken::classify` is reached. The FN side is still protected (pipe-to-shell detection catches the RHS of the synthetic split), but the FP side cannot round-trip through the parser intact. Correct handling requires `src/parser/` extraction to layer redirect tokenization above pipe normalization, deferred to v0.10.0.

### Security

- **`#212` Layer 2 redirect-axis bypass closed**. AI agent invocation paths using redirect operators (`2>&1`, `&>>`, `<>`, `<<-`, fd-prefixed forms, etc.) on a `pipe-to-shell + transparent-wrapper` chain now block consistently across all 9 wrappers (`env`, `sudo`, `timeout`, `nice`, `nohup`, `command`, `exec`, `doas`, `pkexec`). Closure technique: `RedirectToken::token_span()` arity replaces the 2-boolean classifier so `&>>` (PureWithOperand, span=2) consumes operator+operand instead of being misclassified as a single-token concatenated redirect. No GHSA filed (precedent maintenance: v0.9.5 wrapper closure / v0.9.6 env-S closure both shipped without GHSA — operating-model consistency over case-by-case advisory decisions).

### For users

- No installation change. v0.9.8 is a security patch; the moat tightens silently. AI agents that previously could bypass Layer 2 with redirect-suffixed `pipe-to-shell` (`curl URL | env bash 2>&1`, `curl URL | bash &>> log`, etc.) now hit a Block consistently. End-user experience for legitimate commands is unchanged.
- macOS-only. v0.9.8 introduces no platform expansion.
