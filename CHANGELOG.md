...
- **A verification halt no longer hides a deleted tail.** ([#470](https://github.com/yottayoshida/omamori/issues/470), the high-water-mark half)
  ...
  **Residual risk, stated rather than closed**: the compared end is not authenticated, so an attacker who deletes the tail *and* renumbers the line the file then ends on hides the removal again. That is strictly narrower than changing a single character, and it is not equivalent to authentication. **[#470](https://github.com/yottayoshida/omamori/issues/470) stays open** for the rest of what it proposes — `seq` continuity and `prev_hash` linkage across the halted region — which is deliberately not done: in a region the binary cannot authenticate, an attacker writes those fields too, so passing the check would assert nothing, and a future `chain_version` that redefines them would make it fail on genuine logs.
