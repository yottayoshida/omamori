...
- [#184](https://github.com/yottayoshida/omamori/pull/184) — `feat(unwrap): Shell-Layer Hardening Phase 2 (scope 5+6+7)`. Closes 22 bypass variants across 7 wrapper / argv-reorder / env-tampering surfaces. Subagent rounds 1 and 2 caught a raw-state gate-violation in the initial fix that reintroduced a different bypass via helper rename / caller mismatch — captured as the `feedback_fix_creates_new_bypass` discipline. Refs #146 P2.
