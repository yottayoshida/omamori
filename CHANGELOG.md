...
- **README restructured for navigation** ([#191](https://github.com/yottayoshida/omamori/pull/191), [#192](https://github.com/yottayoshida/omamori/pull/192)): the philosophy block is surfaced before Quick Start, and the H2 hierarchy is compressed from 13 to 9 by purifying Quick Start, consolidating tool compatibility, adding a Real-world Effect section, and merging Scope and Limitations. First-time-reader navigation cost is reduced without losing detail.
- **`SECURITY.md` navigation block** ([#192](https://github.com/yottayoshida/omamori/pull/192)): top-of-file index links the principal sections (invariants / known limitations / scope / threat model / audit chain) so readers can jump to a subsection without scrolling.
- **`SECURITY.md` Known Limitations 3-way split** ([#191](https://github.com/yottayoshida/omamori/pull/191)): the previously-mixed table is now grouped into "security gaps planned to close", "observability artefacts (intentional)", and "scope-by-design" so readers can distinguish a hole from intent.
...
- [#191](https://github.com/yottayoshida/omamori/pull/191) — `docs(release): philosophy flip + Known Limitations 3-way split`. README narrative restructure (philosophy block surfaced before Quick Start), `SECURITY.md` Known Limitations split into security-gap / observability-artefact / scope-by-design groups, and CHANGELOG `### Security` narrative balance fix (the prior PR6-bias under-represented PR2's pipe-RHS work). Codex R1-R3 + Claude proxy R4 (4 rounds total); proxy R4 surfaced the narrative-balance gap. Doc-only.
