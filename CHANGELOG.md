...
- **A FIFO placed at a retired-key path no longer hangs omamori.** `open(O_RDONLY)` on a FIFO blocks until someone writes to it, which would stall not just `verify` but every hook-driven audit append — omamori would stop protecting anything, from one `mkfifo` in the data directory. Read-side opens now pass `O_NONBLOCK`, so the open itself cannot wait, and `read_secret` re-checks that what it opened is a regular file and bounds the read at the descriptor. A `stat` pre-check runs first, but only to produce a precise message: a stat followed by an open is a race, and omamori reads this file on every command, so an attacker gets unlimited attempts to win it. ([#457](https://github.com/yottayoshida/omamori/issues/457))
